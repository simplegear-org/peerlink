...
- Регистрация устройства на push-сервере должна обновляться периодически через `PushDeviceRegistrationService`: TTL 24 часа, запуск на startup/resume/connectivity restored и force-регистрация после импорта push-серверов из invite/config; повторная регистрация должна обходить in-memory signature skip через `force`.
...
- Push payload parsing, server-storage merge/update parsing и форматирование push-логов живут в focused helper-ах `lib/core/firebase/firebase_push_*`; не возвращать эти ответственности обратно в `FirebasePushPayloadProcessor`.
...
- Foreground push на iOS/Android не должен показывать системное уведомление: сообщения обновляют UI/счетчики, звонки показывают in-app экран входящего вызова.
- Android standard `call_invite` push должен оставаться data-only; native FCM service показывает fullscreen call notification только когда приложение не активно.
- Android call notifications должны очищаться из status bar при просмотре раздела звонков; пропущенный звонок не должен автоматически становиться seen при записи call history.
...
- После успешной отправки group text/media сообщение должно best-effort публиковать подписанное push-событие в `push.js` через `/events/push`, чтобы сервис разослал `group_update` получателям.
- Push payload для `/events/push` должен включать опциональный блок `servers` с полными endpoint-ами доступных bootstrap/relay/push/turn серверов.
...
- Call runtime декомпозирован шире старой controller-тройки: media readiness/recovery, remote control, renegotiation, video signaling/transceiver/quality, terminal lifecycle, runtime tracking, epoch timers и diagnostics должны оставаться в отдельных `lib/core/calls/call_*` модулях, а не возвращаться в `CallService` или `AudioCallPeer`.
...
- В Settings добавлен отдельный список `Push servers`: экран позволяет добавлять/удалять push endpoint-ы (ввод домена/IP без протокола, runtime сам нормализует в `https://<host>:445`); runtime выбирает валидный endpoint из списка для `/devices/register|unregister` и `/events/push`.
...
- Invite QR/direct-open из Contacts использует `peerlink://invite?payload=...`; мультиязычный текст шаринга содержит две ссылки с одинаковым payload: `peerlink://invite?payload=...` и fallback `https://simplegear.org/invite?payload=...`. Оба формата содержат Peer ID приглашающего и текущую доступную конфигурацию серверов, а импорт должен merge-ить серверы без замены существующих настроек; self-invite должен сначала merge-ить вложенную конфигурацию серверов и только потом пропускать создание контакта.
- `Поделиться конфигурацией` использует мультиязычный текст с `peerlink://config?payload=...` и fallback `https://simplegear.org/config?payload=...` только с текущей доступной конфигурацией серверов. App-side config deep link напрямую merge-ит `bootstrap/relay/turn/push` без QR-диалога выбора режима импорта; QR/manual import сохраняет выбор режима.
...
- Сгруппированная история звонков должна показывать статус последней записи группы, а не суммарный missed count.
...
- Временная недоступность relay для незавершенного incoming media не является terminal failure; retry может продолжаться позже при активном приложении/open-chat/resume/connectivity restore, terminal stop допустим после подтвержденного `blob not found`.
- Android foreground service для incoming media restore не используется, чтобы не запрашивать `FOREGROUND_SERVICE_DATA_SYNC`; не возвращать native background media service без отдельного релизного решения и Play Console disclosure.
...
- Декомпозиция chat-state также включает focused account/direct/group inbound/outbound handler-ы, file/history/cleanup/message coordinator-ы, incoming media restore, outgoing relay-media resume, account payload decoding и reply metadata resolution; эти bounded-ответственности должны оставаться вне `ChatController`.
...
- SQLite-хранилище chat-сообщений уникализирует сообщения по паре `(peerId, messageId)`, а не по глобальному `messageId`, чтобы одинаковые ids в разных чатах не вытесняли сообщения друг друга.
...
- Пересылка сообщений реализуется через `ChatForwardService`: он строит список целей из чатов и контактов, сортирует по свежести сообщений и дублирует текст/медиа через существующие send API. `ChatScreen` должен оставаться wiring-слоем, а медиа-подготовка пересылки не должна блокировать закрытие target sheet.
- Android `video/dolby-vision` / HDR 10-bit видео может быть неподдержано системным decoder-ом; UI должен показывать user-friendly fallback и действие открытия файла во внешнем приложении, а не только raw `PlatformException`.
...
7. Считать `pubspec.yaml` основным источником версии приложения; использовать `tool/bump_version.dart` для синхронизации Android/iOS/macOS version metadata вместо ручного редактирования platform-файлов.
