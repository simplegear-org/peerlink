...
PeerLink keeps versioning intentionally simple: `pubspec.yaml` is the primary source of truth for the app version.
...
Flutter maps this version into native build metadata during builds:
- Android:
  - `versionName <- x.y.z`
  - `versionCode <- n`
- iOS:
...
`tool/bump_version.dart` also syncs local/generated platform files that Xcode/Gradle may read before the next Flutter build:
...
Version changes should not be maintained manually in platform files.
...
The script updates the `version:` line in `pubspec.yaml`, syncs Android/iOS/macOS platform metadata, and prints the resulting version.
