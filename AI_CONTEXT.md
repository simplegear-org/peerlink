...
- Push device registration must be refreshed periodically through `PushDeviceRegistrationService`: 24h TTL, triggers on startup/resume/connectivity restored, and forced registration after importing push servers from invite/config; periodic refresh must bypass the in-memory signature skip via `force`.
...
- Push payload parsing, server-storage merge/update parsing, and push log formatting live in focused `lib/core/firebase/firebase_push_*` helpers; do not put these responsibilities back into `FirebasePushPayloadProcessor`.
...
- Foreground push on iOS/Android must not show system notifications: messages update UI/counters, calls show the in-app incoming-call screen.
- Android standard `call_invite` push must stay data-only; the native FCM service shows fullscreen call notifications only while the app is inactive.
- Android call notifications must be cleared from the status bar when the Calls screen is viewed; missed calls must not become seen automatically when call history is recorded.
...
- Call runtime decomposition is now broader than the old controller trio: keep media readiness/recovery, remote control, renegotiation, video signaling/transceiver/quality, terminal lifecycle, runtime tracking, epoch timers, and diagnostics in their dedicated `lib/core/calls/call_*` modules instead of growing `CallService` or `AudioCallPeer`.
...
- Contacts invite QR/direct-open uses `peerlink://invite?payload=...`; localized share text includes two links with the same payload: `peerlink://invite?payload=...` and fallback `https://simplegear.org/invite?payload=...`. Both formats carry the inviter Peer ID plus currently available server configuration, and imports should merge servers instead of replacing existing settings; self-invite links must still merge the embedded server config before skipping contact creation.
- `Share configuration` uses localized text with `peerlink://config?payload=...` plus fallback `https://simplegear.org/config?payload=...`, containing only currently available server config. App-side config deep links merge `bootstrap/relay/turn/push` directly without the QR import mode dialog; QR/manual import still keeps the explicit mode choice.
...
- Grouped call history should show the latest entry status for the group, not the aggregate missed-call count.
...
- Temporary relay unavailability for unfinished incoming media is not a terminal failure; retries may continue later while the app is active, on chat open, app resume, or connectivity restoration, while confirmed `blob not found` remains a terminal stop.
- Android foreground service is not used for incoming media restore to avoid requesting `FOREGROUND_SERVICE_DATA_SYNC`; do not reintroduce a native background media service without an explicit release decision and Play Console disclosure.
...
- Chat-state split also includes focused account/direct/group inbound/outbound handlers, file/history/cleanup/message coordinators, incoming media restore, outgoing relay-media resume, account payload decoding, and reply metadata resolution; keep these bounded responsibilities outside `ChatController`.
...
- SQLite chat storage must keep uniqueness scoped by `(peerId, messageId)` and changes to chat-message persistence should keep real `StorageService` SQLite write/read test coverage.
...
- Message forwarding is implemented through `ChatForwardService`: it builds targets from chats and contacts, sorts by latest message activity, and duplicates text/media through existing send APIs. `ChatScreen` should stay a wiring layer, and media preparation must not block the target sheet close animation.
- Android `video/dolby-vision` / HDR 10-bit videos may be unsupported by the platform decoder; UI should show a user-friendly fallback and an action to open the file in another app instead of only surfacing raw `PlatformException`.
...
7. Treat `pubspec.yaml` as the primary source of truth for app versioning; use `tool/bump_version.dart` to sync Android/iOS/macOS version metadata instead of editing platform files manually.
