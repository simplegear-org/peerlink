...
- Outgoing messages show receipt checks: 1 overlapped check after confirmed store, 2 after durable receiver save, and 3 after read; in groups, 2/3 checks appear after the first participant delivers/reads the message.
- Chat list rows show top-right receipt checks for the last outgoing message and the last-message time; formatting is: today `HH:MM`, current year `DD:MM`, previous years `DD:MM:YY`.
...
- iOS ATS baseline:
  ...
  - iOS deployment target and Flutter framework `MinimumOSVersion` are `15.0`.
...
- Settings include a `Server sharing via push` section:
  ...
  - receiving is available only while sending is enabled; turning sending off always turns receiving off.
- Settings now include a `Storage` section:
  - top-level block shows `Total app storage`,
  - top-level storage breakdown is cached briefly to avoid repeated expensive scans during rebuilds,
...
- Chat composer behavior:
  - automatically capitalizes sentence starts,
  - uses a multiline keyboard with a newline key,
  - sends messages through the UI send button,
  - raises the list and composer above the keyboard when input starts.
- The chat attachment menu now keeps only implemented actions: `Gallery`, `Paste`, and `Cancel`.
- The message long-press menu now supports forwarding:
  - the target list combines chats and contacts,
  - targets are sorted by latest message activity, with contacts without chats after active chats,
  - text messages and locally available media are duplicated into the selected chat or contact.
...
- Chat media previews:
  - images use generated local thumbnails stored next to managed media,
  - video bubbles show a compact black placeholder with a play overlay instead of initializing a player in every message bubble,
  - image thumbnails are backfilled for older local media when chat history is loaded,
...
- Android `call_invite` in the standard FCM path is data-only; the native handler shows fullscreen call notifications only while the app is inactive.
- Android call notifications are cleared from the status bar after the Calls screen is viewed; missed-call badge count remains until then.
...
- Relay enforces server-side group membership for group fan-out. Blob upload endpoints are storage-only signed writes; group delivery authorization happens on `/relay/group/store`.
