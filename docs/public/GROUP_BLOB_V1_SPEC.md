...
Status: partial implementation. Current runtime already uses `/relay/group/store` plus the blob upload path. Server-side membership checks are active on group delivery (`/relay/group/store`) and membership sync, while blob upload endpoints are storage-only signed writes. Group-key rotation is active.
...
4. Mark local message as sent after `group/event` success.
5. Current dual-stack runtime: if the blob is already uploaded but legacy `/relay/group/store` is rejected because server-side membership is stale, the client may send the encrypted blob reference to members through direct reliable fallback and discard the pending group payload after full success.
...
- `group-members-update owner mismatch` is treated as a terminal pending membership update error and removed from the retry queue.
...
- [done] Enforce membership check on current group delivery path (`/relay/group/store`) and membership sync endpoint (`/relay/group/members/update`). Blob upload endpoints remain storage-only signed writes.
