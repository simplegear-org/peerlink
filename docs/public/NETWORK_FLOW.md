...
main.dart
  ...
  -> AppCompositionRoot.createRuntimeDependencies()
    ...
    -> MeshNode.initialize()
    -> NodeFacade
    -> narrow node capability APIs / ChatRuntimeApi for migrated consumers
    -> AppUiDependencies
      -> app push/deep-link/call/lifecycle/badge coordinators
  -> UiApp
  -> apply persisted server configuration
    -> ServerHealthCoordinator.initialize()
    -> one startup push-device/policy sync
    -> schedule availability refresh in background
  -> AppBootstrapCoordinator.postBootstrap(...) in background
```

The UI does not wait for bootstrap/relay/TURN/push availability probes. Server
configuration is applied before UI creation; health snapshots continue to update
asynchronously.
...
`NodeFacade.sendPayload(targetKind: ChatPayloadTargetKind.direct)` remains the
compatibility entrypoint. New or migrated consumers should depend on narrow
capability contracts such as `MessagingApi`/`NetworkApi` where practical.
...
- UI send status is separate from relay ack: `sendReceipt.sent=true` gives 1 check, incoming `message_receipt delivered` gives 2 checks, and incoming `message_receipt read` gives 3 checks,
...
Relay discovery and object routing are intentionally different:

- configured relay servers are the local Settings-owned pool;
- a peer's advertised relay topology is stored separately in
  `PeerRelayDirectory` (TTL 30 days) and may prioritize a healthy sender/
  receiver intersection for a new direct store;
- successful writes return exact relay locations; `blobRelayServers` is carried
  in direct/group media references and is used first for targeted blob fetch;
- incoming `ChatService` metadata updates only the peer directory. It never
  merges foreign relay endpoints into the persistent configured pool.

Message receipt path:
- after durably saving an incoming direct/group message or media placeholder, the receiver best-effort sends the author a direct reliable `__peerlink_message_receipt_v1__` payload with status `delivered`;
- after `ChatReadStateService.markChatAsRead(...)`, the receiver sends `read` receipts for the incoming messages that were actually marked read;
- for groups, receipts are sent to the message author and sender UI aggregates by `at least one member`: non-empty `deliveredAtByPeer` = 2 checks, non-empty `readAtByPeer` = 3 checks;
- receipt payloads are classified as service events and are not rendered in chat history.
...
- push event path:
  - after a successful runtime action, the app may emit signed `/events/push` to `push.js`,
  - the event includes `recipientUserIds`, arbitrary app-defined `payload`, optional `notification`, and optional `delivery`,
  - the server validates the signature and performs fanout by recipient device tokens without rewriting `payload`,
  - access-policy schema v2 carries separate direct/group message/call mute
    lists; the server suppresses only matching notification fanout, while the
    reliable relay envelope is still stored and fetched normally,
  - FCM payload includes both `notification` and `data/payload` fields for better iOS background visibility,
  - the client uses such push as an accelerator, but still runs `pollRelay()` on startup, resume, and connectivity restore.

- incoming group relay payload path:
  - `ReliableRelayPollController` forwards `groupId` into the inbound pipeline,
  - `ReliableInboundProcessor` stores `groupId` in `ReliableMessageEnvelope`,
  - `ChatService` publishes `messageReceived` into the group chat target when `groupId` is present instead of routing it to the sender peer.

Personal media path (current):
- direct chat media uploads the file to relay blob storage first,
- before upload, up to 3 live relays are selected and the blob is replicated to
  every selected available relay. One-shot upload is parallel; large chunked
  upload is sequential per relay (up to 5 chunks concurrently within one relay),
- a chunk upload makes one request with a five-second timeout, then immediately
  fails over to the next relay; the failed relay is locally cooled down for two
  minutes even if shared health is still stale. The blob is stored on every
  remaining available relay. Replication progress does not restart per relay
  and reaches 100% only at the end,
- sender then delivers encrypted `direct_blob_ref` metadata, including the
  exact `blobRelayServers` when available, through the normal personal reliable-message channel,
...
- direct blob download first uses `blobRelayServers` without changing local
  relay configuration, then keeps the legacy configured-pool fallback for old
  references; it also uses retry/timeout protection,
...
- if incoming media restore fails during a transient network switch or temporary relay unavailability, the client keeps retrying later while the app is active,
...
- Android foreground service is not used for media restore, so `FOREGROUND_SERVICE_DATA_SYNC` is not required,
...
- incoming relay media rejects delayed outgoing (`transfer.outgoing.*`) progress
  statuses, so it cannot show `Sending 100%` after download,
- after download reaches 100%, the visible stage advances through download, decrypt, and save; successful message persistence clears the transfer status,
- only download/relay availability failures are retryable; decrypt, save, and message-update failures are terminal and have distinct UI statuses,
- `localFilePath` is persisted before thumbnail generation; thumbnails are best-effort and cannot restart relay download or invalidate restored media,
...
- multi-relay fetch retains exact envelope locations. ACK is sent in parallel to
  every known replica, even if it is outside the local configured relay pool.
  A partial ACK does not invalidate delivery or the committed cursor; failed
  cleanup is retried on a later poll/reconnect and replica TTL is the fallback.
- ACK removes the message envelope only. A referenced media blob remains until
  its independent blob TTL expires.
