...
В PeerLink версионирование специально держится простым: основной источник версии приложения — `pubspec.yaml`.
...
Flutter при сборке прокидывает эту версию в нативные метаданные:
- Android:
  - `versionName <- x.y.z`
  - `versionCode <- n`
- iOS:
...
Дополнительно `tool/bump_version.dart` синхронизирует локальные/generated platform-файлы, которые Xcode/Gradle могут читать до следующей Flutter-сборки:
...
Скрипт меняет строку `version:` в `pubspec.yaml`, проливает версию в Android/iOS/macOS platform metadata и выводит итоговую версию.
