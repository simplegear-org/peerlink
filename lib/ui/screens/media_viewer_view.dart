...
class _MediaVideoPageState extends State<MediaVideoPage> {
  ...
  Future<void> _prepareVideo() async {
    ...
    if (bytes == null) {
      throw StateError('Видео еще не загружено');
    }
    ...
    return FutureBuilder<void>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError || _videoController == null) {
          return Center(
            child: Text(
              'Не удалось открыть видео: ${snapshot.error ?? "источник видео недоступен"}',
            ...
            Positioned(
              ...
              child: ValueListenableBuilder<VideoPlayerValue>(
                ...
                builder: (context, value, _) {
                  ...
                  final sliderValue = currentMs.toDouble().clamp(0.0, sliderMax);

                  return Container(
                    padding: MediaViewerStyles.controlsPadding,
                    decoration: BoxDecoration(
                      color: Colors.black.withValues(alpha: 0.55),
                      borderRadius: BorderRadius.circular(MediaViewerStyles.controlsRadius),
                      border: Border.all(
                      ...
                      children: [
                        SliderTheme(
                          data: SliderTheme.of(context).copyWith(
                            ...
                            overlayColor: AppTheme.accentSoft.withValues(alpha: 0.18),
                            trackHeight: 3,
                          ),
                          child: Slider(
                            value: sliderValue,
