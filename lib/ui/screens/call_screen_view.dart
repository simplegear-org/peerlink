...
class CallScreenContent extends StatelessWidget {
              ...
              Wrap(
                ...
                children: [
                  StatPill(
                    ...
                    value: _statusLabel(context.strings),
                    fontSize: metaFont,
                  ...
                  if (dataBytesListenable != null)
                    ValueListenableBuilder<int>(
                      ...
                      builder: (context, bytes, child) {
                        return StatPill(
                          label: context.strings.channel,
                          value:
                              '${_transportBadge(state)} ${SettingsScreenFormatters.formatBytes(bytes)}${_videoCodecSuffix(state)}',
                          fontSize: metaFont,
                        );
                      },
                    )
                    ...
                    StatPill(
                      label: context.strings.channel,
                      value:
                          '${_transportBadge(state)} ${SettingsScreenFormatters.formatBytes(currentDataBytes)}${_videoCodecSuffix(state)}',
                      fontSize: metaFont,
                    ),
                ],
              ),
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(
              CallScreenStyles.middleIconRadius,
            ),
          ...
          children: [
            ...
            Positioned(
              ...
              child: SizedBox(
                ...
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(
                    CallScreenStyles.previewRadius,
                  ),
                  child: DecoratedBox(
                    decoration: BoxDecoration(
            ...
            children: [
              Expanded(
                child: FilledButton.icon(
                  ...
                  label: Text(context.strings.answer),
                ),
              ),
    ...
    return Center(
      child: Column(
        ...
        children: [
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: const Color(
                      0xFF7AE582,
                    ).withValues(alpha: 0.05 + (0.08 * (1 - pulse))),
                  ),
                ),
                Container(
                  width: middleSize,
          ...
          Text(
            _progressTitle(context.strings),
          ...
          Text(
            _progressSubtitle(context.strings),
            textAlign: TextAlign.center,
  ...
  bool get _hasLocalVideo =>
      ...
      (state.localStream?.getVideoTracks().isNotEmpty ?? false);
  ...
  bool get _showsRemoteVideoStage =>
      state.remoteVideoEnabled || state.remoteVideoActive;
  bool get _hasRemoteVideo =>
      _showsRemoteVideoStage && state.remoteStream != null;
  bool get _showsVideoStage => _hasLocalVideo || _showsRemoteVideoStage;
  MediaStream? get _localPreviewStream =>
      _hasLocalVideo ? state.localStream : null;
  MediaStream? get _remoteDisplayStream =>
      _hasRemoteVideo ? state.remoteStream : null;
  ...
  String _progressTitle(AppStrings strings) {
    switch (state.phase) {
      case CallPhase.outgoingRinging:
        return strings.calling;
      case CallPhase.incomingRinging:
        return strings.incomingCall;
      case CallPhase.connecting:
        return strings.establishingConnection;
      case CallPhase.recovering:
        return contactName;
      case CallPhase.active:
      case CallPhase.ended:
  ...
  String _progressSubtitle(AppStrings strings) {
    switch (state.phase) {
      case CallPhase.outgoingRinging:
        return strings.waitingForAnswer;
      case CallPhase.incomingRinging:
        return strings.peerCallingYou;
      case CallPhase.connecting:
        return strings.preparingCallTransport;
      case CallPhase.recovering:
        return state.debugStatus ?? strings.preparingCallTransport;
      case CallPhase.active:
      case CallPhase.ended:
      case CallPhase.failed:
      case CallPhase.idle:
  ...
  String _statusLabel(AppStrings strings) {
    switch (state.phase) {
      case CallPhase.incomingRinging:
        return strings.incoming;
      case CallPhase.outgoingRinging:
        return strings.dialling;
      case CallPhase.connecting:
        return strings.connecting;
      case CallPhase.recovering:
        return state.debugStatus ?? strings.connecting;
      case CallPhase.active:
        final duration = _formatDuration();
        return duration == '00:00' ? strings.connected : duration;
      case CallPhase.ended:
