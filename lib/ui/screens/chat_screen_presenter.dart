...
class ChatScreenPresenter {
  ...
  String? replySenderLabelForPreview(Message message) {
    final senderPeerId = message.replyToSenderPeerId?.trim();
    if (senderPeerId == null || senderPeerId.isEmpty) {
      final fallback = message.replyToSenderLabel?.trim();
      return fallback == null || fallback.isEmpty ? null : fallback;
    }
    if (senderPeerId == controller().localPeerId) {
      return strings().you;
    }
    final contactName = controller().contactNameForPeer(senderPeerId)?.trim();
    if (contactName != null && contactName.isNotEmpty) {
      return contactName;
    }
    if (!isGroupChat() && senderPeerId == chat().peerId) {
      final chatName = chat().name.trim();
