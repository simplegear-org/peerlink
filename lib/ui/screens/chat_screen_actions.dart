...
enum ChatAttachAction { gallery, paste }
...
class ChatScreenActions {
  ...
  Future<ChatForwardTarget?> showForwardTargetSheet({
    required BuildContext context,
    required List<ChatForwardTarget> targets,
  }) {
    return showModalBottomSheet<ChatForwardTarget>(
      context: context,
  ...
  Future<void> pickAndSetGroupAvatar({
    ...
    try {
      await controller.setGroupAvatar(
        groupId: groupId,
        bytes: cropped,
        mimeType: ChatScreenMimeType.forPath(
          selected.name,
          selectedFileNameFallback,
        ),
      );
      if (!context.mounted) {
        return;
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  onTap: () {
                    Navigator.of(context).pop(ChatMessageAction.forward);
                  },
                ),
                if (canRetry)
                  ListTile(
