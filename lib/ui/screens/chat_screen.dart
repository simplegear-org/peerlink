...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  int _slowFrameBurstCount = 0;
  int _worstSlowFrameTotalMs = 0;
  int _worstSlowFrameBuildMs = 0;
  int _worstSlowFrameRasterMs = 0;
            ...
            Expanded(
              child: ChatScreenMessageList(
                ...
                unreadDividerMessageId:
                    _scrollCoordinator.unreadDividerMessageId,
                ...
                replySenderLabelFor: _presenter.replySenderLabelForPreview,
  ...
  void dispose() {
    developer.log(
      '[chat_ui] dispose unload peer=${widget.chat.peerId} '
      'messages=${widget.chat.messages.length} '
      'loaded=${widget.chat.messagesLoaded}',
      name: 'chat',
      level: 900,
    );
    unawaited(widget.controller.unloadChatMessages(widget.chat.peerId));
    ...
    developer.log(
      '[chat_perf] dispose clear-image-cache peer=${widget.chat.peerId} '
      'current=${imageCache.currentSize} '
      'live=${imageCache.liveImageCount} '
      'bytes=${imageCache.currentSizeBytes}',
      name: 'chat',
      level: 900,
    );
  ...
  void _handleFrameTimings(List<FrameTiming> timings) {
    for (final timing in timings) {
      ...
      if (totalMs < 24 && buildMs < 16 && rasterMs < 16) {
        continue;
      }
      _slowFrameBurstCount += 1;
      if (totalMs > _worstSlowFrameTotalMs) {
        _worstSlowFrameTotalMs = totalMs;
        _worstSlowFrameBuildMs = buildMs;
        _worstSlowFrameRasterMs = rasterMs;
      }
    }
    if (_slowFrameBurstCount == 0) {
      return;
    }
    final now = DateTime.now();
    final previous = _lastSlowFrameLogAt;
    if (previous != null && now.difference(previous).inSeconds < 2) {
      return;
    }
    _lastSlowFrameLogAt = now;
    developer.log(
      '[chat_perf] slow-frame peer=${widget.chat.peerId} '
      'count=$_slowFrameBurstCount '
      'worstTotalMs=$_worstSlowFrameTotalMs '
      'worstBuildMs=$_worstSlowFrameBuildMs '
      'worstRasterMs=$_worstSlowFrameRasterMs '
      'messages=${widget.chat.messages.length}',
      name: 'chat',
      level: 900,
    );
    _slowFrameBurstCount = 0;
    _worstSlowFrameTotalMs = 0;
    _worstSlowFrameBuildMs = 0;
    _worstSlowFrameRasterMs = 0;
  }
  ...
  void didChangeMetrics() {
    ...
    final wasNearBottom = _scrollCoordinator.isNearBottom(threshold: 320);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (keyboardOpened && wasNearBottom) {
        unawaited(_scrollCoordinator.jumpToBottomAfterLayout(settle: true));
      }
      setState(() {});
    });
  }
  ...
  Future<void> _forwardMessage(Message message) async {
    final targets = _forwardService.targets(
      chats: widget.controller.getChatsSorted(),
      contacts: widget.controller.getContacts(),
    );
    if (targets.isEmpty) {
      if (!mounted) {
        return;
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(context.strings.noForwardTargets)));
      return;
    }

    final target = await _screenActions.showForwardTargetSheet(
      context: context,
      targets: targets,
    );
    if (target == null) {
    ...
    unawaited(_forwardMessageToTarget(message, target));
  }
  ...
  Future<void> _forwardMessageToTarget(
    ...
    try {
      if (!target.isGroup) {
        await widget.controller.createDirectChat(
          peerId: target.peerId,
          name: target.name,
        );
      }
      await _forwardService.forward(
        message: message,
        target: target,
        sendMessage: (peerId, text) =>
            widget.controller.sendMessage(peerId, text),
        sendFile:
            (
              peerId, {
              required fileName,
        ...
        ensureLocalMedia: _ensureForwardSourceMedia,
      );
      if (!mounted) {
        return;
      }
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(context.strings.forwarded)));
    } catch (error, stackTrace) {
      developer.log(
        'forward failed messageId=${message.id} error=$error',
        name: 'chat',
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(context.strings.forwardFailed(error))),
      );
    }
  }

  Future<String?> _ensureForwardSourceMedia(Message message) async {
    final path = message.localFilePath?.trim();
    if (path != null && path.isNotEmpty && await File(path).exists()) {
      return path;
    }
    final transferId = message.transferId ?? '';
    if (transferId.startsWith('grpblob:')) {
      return widget.controller.restoreGroupBlobMedia(message);
    }
    if (transferId.startsWith('dirblob:')) {
      return widget.controller.restoreDirectBlobMedia(message);
    }
    return null;
  }
