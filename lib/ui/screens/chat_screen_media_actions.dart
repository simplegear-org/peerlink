...
class ChatScreenMediaActions {
  ...
  Future<void> handleFileTap({
    ...
    final canRestoreFromBytes = !hasLocalFile &&
        hasEmbeddedBytes &&
        activeMessage.kind == MessageKind.file;

    final isDownloadingIncomingMedia = activeMessage.incoming &&
        activeMessage.kind == MessageKind.file &&
        (activeMessage.transferStatus?.contains('Получ') ?? false);
    if (isDownloadingIncomingMedia) {
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Медиафайл еще загружается.')),
      );
      return;
    }
    ...
    if (activeMessage.isMedia && (hasLocalFile || hasEmbeddedBytes)) {
      ...
      final initialIndex = mediaMessages.indexWhere((item) => item.id == activeMessage.id);
      if (!context.mounted) return;
          ...
          if (await restoredFile.exists()) {
            ...
            if (refreshed.isMedia) {
              final mediaMessages = chat.messages
                  ...
                  .toList(growable: false);
              final initialIndex =
                  mediaMessages.indexWhere((item) => item.id == refreshed.id);
              await Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => MediaViewerScreen(
                    mediaMessages: mediaMessages,
    ...
    if (activeMessage.isMedia) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Медиафайл недоступен на устройстве.')),
      );
      return;
    }
  ...
  Future<void> openFileMessage({
    ...
    if (file == null || !await file.exists()) {
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Файл недоступен для открытия.')),
      );
      return;
    }
  ...
  Future<void> pickAndSendGalleryMedia({
    ...
    for (final file in result.files) {
      ...
      if (file.size > maxFileSize) {
        showPlaceholder('Файл больше 1 ГБ не поддерживается.');
        continue;
      }
    ...
    if (failedCount > 0) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            firstError == null
                ? 'Не удалось добавить $failedCount из $total файлов.'
                : 'Не удалось добавить $failedCount из $total файлов: $firstError',
          ),
        ),
      );
    }
