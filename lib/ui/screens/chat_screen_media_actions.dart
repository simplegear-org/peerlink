...
import '../widgets/message_file_availability_cache.dart';
...
class ChatScreenMediaActions {
        ...
        try {
          ...
          if (restoredPath != null && restoredPath.isNotEmpty) {
            ...
            if (await restoredFile.exists()) {
              MessageFileAvailabilityCache.update(restoredPath, true);
              final refreshed = _currentMessage(
                chat,
                message.id,
                fallback: activeMessage,
              );
              if (!context.mounted) return;
  ...
  Message _currentMessage(
    ...
    return chat.messages.firstWhere(
