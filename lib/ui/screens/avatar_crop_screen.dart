...
class _AvatarCropScreenState extends State<AvatarCropScreen> {
  ...
  Future<void> _decodeImage() async {
    try {
      ...
      _error = 'Не удалось открыть изображение: $error';
    } finally {
      if (mounted) {
        setState(() {
          _busy = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null || image == null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Кадрирование')),
        body: Center(
          child: Padding(
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Кадрирование аватара')),
      body: LayoutBuilder(
...
class _AvatarImagePainter extends CustomPainter {
  ...
  bool shouldRepaint(covariant _AvatarImagePainter oldDelegate) {
    ...
    for (var i = 0; i < 16; i++) {
      if ((oldDelegate.transform.storage[i] - transform.storage[i]).abs() > 1e-6) {
        return true;
      }
    }
    return false;
