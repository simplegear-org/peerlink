...
class SettingsScreenSystemActions {
  ...
  Future<void> shareServerConfigQr(BuildContext context) async {
    await SharePlus.instance.share(
      ShareParams(
        text: controller.exportServerConfigShareText(context.strings),
      ),
    );
  }
  ...
  Future<void> installSelfHostedServers(BuildContext context) async {
    ...
    if (request.host.isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(context.strings.fillServerLoginPassword)),
      );
      return;
    }
              ...
              Future<void>.microtask(() async {
                ...
                try {
                  final result = await selfHostedDeployService.deploy(
                    ...
                    onStage: (event) => appendLog(
                      strings.deployStage(
                        event.stage,
                        event.totalStages,
                        strings.deployStageMessage(event),
                      ),
                    ),
                    onReadiness: (event) =>
                        appendLog(strings.deployReadiness(event)),
                  );
                  ...
                  completionError = _deployErrorText(strings, error);
                  appendLog(strings.deployErrorLog(completionError!));
                  setDialogState(() {
                    completed = true;
                  });
                } finally {
  ...
  String _deployErrorText(AppStrings strings, Object error) {
    if (error is SelfHostedDeployInputException) {
      return strings.deployInputError(error.error);
    }
    if (error is SelfHostedDeployException) {
      final base = strings.deployServiceError(error.error);
      final details = error.details.trim();
      if (details.isEmpty) {
        return base;
      }
      return '$base\n${strings.remoteCommandLastLines(details)}';
    }
    if (error is SelfHostedRemoteCommandException) {
      final base = strings.remoteCommandFailed(error.exitCode);
      final tail = error.lastLines.trim();
      if (tail.isEmpty) {
        return base;
      }
      return '$base\n${strings.remoteCommandLastLines(tail)}';
    }
    return '$error';
  }
