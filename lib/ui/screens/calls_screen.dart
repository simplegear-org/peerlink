...
class _CallsScreenState extends State<CallsScreen> {
  ...
  List<_CallListItem> _groupedEntries(List<CallLogEntry> entries) {
    final grouped = <String, _CallListItem>{};
    final ungrouped = <_CallListItem>[];
    for (final entry in entries) {
      final peerId = entry.peerId.trim();
      if (peerId.isEmpty) {
        ungrouped.add(_CallListItem(entries: <CallLogEntry>[entry]));
        continue;
      }
      final existing = grouped[peerId];
      if (existing == null) {
        grouped[peerId] = _CallListItem(entries: <CallLogEntry>[entry]);
      } else {
        existing.entries.add(entry);
      }
    }
    final items = <_CallListItem>[...grouped.values, ...ungrouped];
    for (final item in items) {
      item.entries.sort((a, b) => b.endedAt.compareTo(a.endedAt));
    }
    items.sort((a, b) => b.primary.endedAt.compareTo(a.primary.endedAt));
    return items;
  }
