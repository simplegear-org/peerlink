...
class _CallsScreenState extends State<CallsScreen> {
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return CallHistoryTile(
                      key: ValueKey(
                        'call-${item.primary.id}-${item.missedCount}',
                      ),
                      data: data,
                      onTap: () => _redial(item),
  ...
  Future<void> _redial(_CallListItem item) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Не удалось начать вызов: $error')),
      );
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _deleteEntry(_CallListItem item) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          item.missedCount > 1
              ? 'Группа пропущенных звонков удалена из истории'
        ...
        return AlertDialog(
          ...
          content: Text(
            item.missedCount > 1
                ...
                : 'Запись звонка будет удалена из истории.',
          ),
          actions: [
            TextButton(
  ...
  String _subtitle(_CallListItem item) {
    ...
    if (item.missedCount > 1) {
      return '$direction • ${item.missedCount} пропущенных подряд';
    }
