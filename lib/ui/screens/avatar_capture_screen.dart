...
class _AvatarCaptureScreenState extends State<AvatarCaptureScreen> {
  ...
  Future<void> _initializeCamera() async {
    try {
      ...
      _error = 'Не удалось открыть фронтальную камеру: $error';
    } finally {
      if (mounted) {
        setState(() {
          _initializing = false;
  ...
  Future<void> _capture() async {
    ...
    try {
      ...
      setState(() {
        _error = 'Не удалось сделать снимок: $error';
      });
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Снимок аватара')),
      body: _initializing
                    ...
                    Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: FilledButton.icon(
                          ...
                          label: Text(_capturing ? 'Сохраняем...' : 'Сделать снимок'),
                        ),
                      ),
                    ),
                  ],
