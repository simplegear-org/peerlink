...
import 'relay_media_transfer_service.dart';

enum ChatConnectionStatus { disconnected, connecting, connected, error }

const List<int> _groupMediaCipherMagicV2 = <int>[
  0x50,
  0x4C,
  0x47,
  0x32,
...
class _IncomingGroupChatDeletePayload {
  final String groupId;
  final String groupName;
  final List<String> memberPeerIds;
  final String? ownerPeerId;
  ...
  const _IncomingGroupChatDeletePayload({
    required this.groupId,
    required this.groupName,
    required this.memberPeerIds,
    required this.ownerPeerId,
...
class _IncomingGroupSecurePayload {
  ...
  const _IncomingGroupSecurePayload({required this.groupId, required this.raw});
}
...
final class _IncomingGroupChatDeleteDispatch extends _IncomingChatDispatch {
  final _IncomingGroupChatDeletePayload payload;

  const _IncomingGroupChatDeleteDispatch(this.payload);
}
...
class ChatController with WidgetsBindingObserver {
  ...
  static const String _groupChatDeletePrefix =
  ...
  final Map<String, Future<String?>> _incomingMediaActiveRestores =
  ...
  Future<void> _loadChats() async {
    ...
    for (final raw in summaries) {
      try {
        ...
        if (_isGroupDeleted(chat.peerId)) {
  ...
  void _loadGroupMetaFromSettings() {
    ...
    final raw =
        _groupMetaBox.get(_groupMetaStorageKey) ??
  ...
  Future<bool> loadMoreMessages(String peerId) async {
    ...
    final olderMessages = await _readOlderMessages(
      peerId,
      currentCount,
      _paginationLimit,
    );
  ...
  Future<List<Message>> _readOlderMessages(
    String peerId,
    int endIndex,
    int limit,
  ) async {
  ...
  _IncomingChatDispatch _classifyIncomingMessage(ChatMessage msg) {
    ...
    final groupChatDelete = _normalizeGroupChatDeletePayload(msg.text);
    if (groupChatDelete != null) {
      return _IncomingGroupChatDeleteDispatch(groupChatDelete);
    }
    ...
    switch (dispatch) {
      case _IncomingDeleteDispatch():
        if (msg.text.isNotEmpty) {
          ...
          final removed = await _removeMessageWithMediaCleanup(
            msg.peerId,
            msg.text,
          );
          if (!removed) {
      ...
      case _IncomingProfileAvatarRemoveDispatch():
        await _avatarService.handleIncomingAvatarRemoval(msg.peerId, msg.text);
        return;
      case _IncomingProfileAvatarQueryDispatch():
        await _avatarService.handleIncomingAvatarQuery(msg.peerId, msg.text);
        return;
      ...
      case _IncomingGroupDeleteDispatch(payload: final payload):
        ...
        return;
      case _IncomingGroupChatDeleteDispatch(payload: final payload):
        await _handleIncomingGroupChatDelete(msg, payload: payload);
        return;
  ...
  bool _isGroupChatDeletePayload(String text) {
    return text.startsWith(_groupChatDeletePrefix);
  }
  ...
  Map<String, dynamic>? _decodeGroupChatDeletePayload(String text) {
    if (!_isGroupChatDeletePayload(text)) {
      return null;
    }
    try {
      final raw = text.substring(_groupChatDeletePrefix.length);
      final decoded = jsonDecode(raw);
      if (decoded is Map<String, dynamic>) {
        return decoded;
      }
  ...
  String _encodeGroupChatDeletePayload(Chat groupChat) {
    final payload = <String, dynamic>{
    ...
    return '$_groupChatDeletePrefix${jsonEncode(payload)}';
  }
  ...
  _parseGroupBlobTransferId(String? transferId) {
  ...
  _parseDirectBlobTransferId(String? transferId) {
    final raw = transferId?.trim() ?? '';
  ...
  _IncomingGroupChatDeletePayload? _normalizeGroupChatDeletePayload(
    String text,
  ) {
    final raw = _decodeGroupChatDeletePayload(text);
    if (raw == null) {
      return null;
    }
    final groupId = (raw['groupId'] as String? ?? '').trim();
    ...
    return _IncomingGroupChatDeletePayload(
      groupId: groupId,
      groupName: (raw['groupName'] as String? ?? '').trim(),
      memberPeerIds: memberPeerIds,
      ownerPeerId: (raw['ownerPeerId'] as String?)?.trim(),
  ...
  _IncomingGroupSecurePayload? _normalizeGroupSecurePayload(String text) {
    ...
    return _IncomingGroupSecurePayload(groupId: groupId, raw: raw);
  }
  ...
  Future<void> _handleIncomingGroupInvite(
    ...
    if (_isGroupDeleted(groupId)) {
      developer.log(
        'group invite drop: group is deleted group=$groupId from=${msg.peerId}',
        name: 'chat',
      );
      return;
    }
  ...
  Future<void> _handleIncomingGroupSecureMessage(
    ...
    if (_isGroupDeleted(groupId)) {
      developer.log(
        'group secure message drop: group is deleted group=$groupId from=${msg.peerId}',
        name: 'chat',
      );
      return;
    }
    ...
    _newMessageNotificationController.add(
      ChatMessage(id: msg.id, peerId: groupId, text: clearText),
    );
    NotificationService.instance
        .showMessageNotification(
  ...
  Future<void> _handleIncomingGroupMessage(
    ...
    if (_isGroupDeleted(groupId)) {
      developer.log(
        'group message drop: group is deleted group=$groupId from=${msg.peerId}',
        name: 'chat',
      );
      return;
    }
    ...
    _newMessageNotificationController.add(
      ChatMessage(id: msg.id, peerId: groupId, text: text),
    );
    NotificationService.instance
        .showMessageNotification(
  ...
  Future<void> _handleIncomingGroupChatDelete(
    ChatMessage msg, {
    _IncomingGroupChatDeletePayload? payload,
  }) async {
    final resolvedPayload =
        payload ?? _normalizeGroupChatDeletePayload(msg.text);
    if (resolvedPayload == null) {
      developer.log(
        'group chat delete drop: invalid payload from=${msg.peerId} id=${msg.id}',
        name: 'chat',
      );
      return;
    }
    ...
    if (groupId.isEmpty) {
      developer.log(
        'group chat delete drop: missing groupId from=${msg.peerId} id=${msg.id}',
        name: 'chat',
      );
      return;
    }

    if (!_isTrustedGroupChatDeleteOwner(msg.peerId, resolvedPayload)) {
      developer.log(
        'group chat delete drop: sender is not owner group=$groupId sender=${msg.peerId}',
        name: 'chat',
      );
      return;
    }
    ...
    await _deleteChatLocal(
      ...
      deletedByPeerId: msg.peerId,
    );
    developer.log(
      'group chat delete applied group=$groupId from=${msg.peerId}',
      name: 'chat',
    );
  }
  ...
  bool _isTrustedGroupChatDeleteOwner(
    ...
    final knownOwner = _knownGroupOwnerPeerId(payload.groupId);
  ...
  Future<void> _handleIncomingGroupMembersUpdate(
    ...
    if (_isGroupDeleted(groupId)) {
      developer.log(
        'group members drop: group is deleted group=$groupId from=${msg.peerId}',
        name: 'chat',
      );
      return;
    }
    if (action == 'leave') {
      await _handleIncomingGroupLeave(msg, payload: resolvedPayload);
      return;
    }
    ...
    final selfRemoved =
        action == 'remove' && changedPeerIds.contains(facade.peerId);
    if (!selfRemoved) {
      members.add(facade.peerId);
    }
    final chat =
  ...
  Future<void> _handleIncomingGroupLeave(
    ChatMessage msg, {
    required _IncomingGroupMembersPayload payload,
  }) async {
    final groupId = payload.groupId;
    final leavingPeerId = msg.peerId.trim();
    ...
    if (ownerPeerId == facade.peerId) {
      await _syncGroupMembershipWithRelay(chat);
      await _rotateGroupKey(chat, recipients: chat.memberPeerIds);
  ...
  Future<void> _handleIncomingGroupBlobRef(
    ...
    await _appendMessage(groupId, incoming);
    _messageUpdatesController.add(groupId);
    _restoreMediaInBackground(incoming, isGroup: true);
    _newMessageNotificationController.add(
      ChatMessage(
        id: blobRef.messageId,
        peerId: groupId,
    ...
    if (existingIndex == -1) {
      chat.messages.add(
        Message(
          ...
          replyToKind: msg.replyToKind,
          transferredBytes: 0,
          transferStatus: _incomingRelayFetchStatus,
          status: MessageStatus.sent,
          isRead: false,
        ),
      );
      await _persistLoadedChat(peerId);
      _messageUpdatesController.add(peerId);
      _restoreMediaInBackground(chat.messages.last, isGroup: false);
    } else if (_shouldAutoRestoreIncomingMedia(chat.messages[existingIndex])) {
      _messageUpdatesController.add(peerId);
      _restoreMediaInBackground(chat.messages[existingIndex], isGroup: false);
    } else {
      _messageUpdatesController.add(peerId);
    }
  ...
  Future<Chat> _createGroupChatInternal({
    ...
    await _syncGroupMembershipWithRelay(chat);
    if (sendInvites) {
      await _sendGroupInvites(chat, recipients: invitees);
      await _rotateGroupKey(chat, recipients: allMembers);
    }
    _messageUpdatesController.add(groupId);
    return chat;
  }
  ...
  Future<void> _sendGroupInvites(
    ...
    for (var i = 0; i < targetRecipients.length; i++) {
      ...
      final messageId =
          'invite:${groupChat.peerId}:${_nextLocalMessageId()}:$i';
      try {
        developer.log(
  ...
  Future<void> addGroupParticipants({
    ...
    await _syncGroupMembershipWithRelay(chat);
    await _rotateGroupKey(chat, recipients: chat.memberPeerIds);
    await _sendGroupInvites(chat, recipients: additions);
    await _broadcastGroupMembersUpdate(
      groupChat: chat,
  ...
  Future<void> _broadcastGroupChatDelete(Chat groupChat) async {
    final recipients = <String>{..._collectGroupRecipients(groupChat)};
    final ownerPeerId = groupChat.ownerPeerId?.trim();
    if (ownerPeerId != null &&
    ...
    final payload = _encodeGroupChatDeletePayload(groupChat);
  ...
  Future<void> _drainFileQueue() async {
    if (_fileSendInFlight) {
      _logQueue('skip drain already in flight queued=${_fileSendQueue.length}');
      return;
    }
    ...
    try {
      while (_fileSendQueue.isNotEmpty) {
        ...
        _logQueue(
          'start peer=${item.peerId} messageId=${item.messageId} remaining=${_fileSendQueue.length}',
        );
        ...
        _logQueue('done peer=${item.peerId} messageId=${item.messageId}');
        _refreshQueuedFileStatuses();
      }
    } finally {
      _activeFileTransferId = null;
      _fileSendInFlight = false;
      _logQueue('idle queued=${_fileSendQueue.length}');
      _refreshQueuedFileStatuses();
    }
  }
  ...
  void _resumeRecoverableFileQueue() {
    ...
    for (final chat in chats.values) {
      ...
      recovered += _recoverPendingFileTransfersForChat(chat);
    }
    _logQueue(
      'resume recovered=$recovered queued=${_fileSendQueue.length} inFlight=$_fileSendInFlight',
    );
  ...
  int _recoverPendingFileTransfersForChat(Chat chat) {
    ...
    for (var i = 0; i < chat.messages.length; i++) {
      ...
      if (!_isRecoverableOutgoingFile(message)) {
        continue;
      }
      ...
      if (_isFileQueuedOrActive(message.id)) {
        continue;
      }
      _fileSendQueue.add(
        QueuedFileTransfer(
          ...
          replyTo: _rebuildReplyMessage(chat.peerId, message),
        ),
      );
      recovered += 1;
      changed = true;
    ...
    if (changed) {
      _schedulePersistLoadedChat(chat.peerId);
      _messageUpdatesController.add(chat.peerId);
    }
    if (recovered > 0) {
      _logQueue('recover peer=${chat.peerId} recovered=$recovered');
      _refreshQueuedFileStatuses();
      unawaited(_drainFileQueue());
    }
    return recovered;
  ...
  Future<void> _sendGroupMessageAsync(Chat groupChat, Message message) async {
    ...
    if (!groupChat.memberPeerIds.contains(facade.peerId)) {
      await _updateMessageStatusById(
        groupChat.peerId,
        message.id,
  ...
  Future<void> _sendFileAsync(
    ...
    _logQueue(
      'upload prepare peer=$peerId messageId=$messageId file=$fileName size=$fileSizeBytes path=${filePath?.isNotEmpty == true} bytes=${fileBytes?.length ?? 0}',
    );
    await _updateFileProgress(
      peerId,
      messageId,
    ...
    try {
      ...
      _logQueue('send ref done peer=$peerId messageId=$messageId');
      ...
      _logQueue('failed peer=$peerId messageId=$messageId error=$e');
  ...
  Future<void> _deleteChatLocal(
    ...
    final loadedChat = chats[peerId];
    final groupMeta = _groupMetaByGroupId[peerId];
    final isGroupChat =
        loadedChat?.isGroup == true ||
    ...
    } else if (_groupMetaByGroupId.remove(peerId) != null) {
      await _persistGroupMetaToSettings();
    }
  ...
  Future<String?> restoreGroupBlobMedia(Message message) {
    return _restoreIncomingRelayMediaOnce(message, isGroup: true);
  }
  ...
  Future<String?> restoreDirectBlobMedia(Message message) {
    return _restoreIncomingRelayMediaOnce(message, isGroup: false);
  }
  ...
  Future<String?> _restoreIncomingRelayMediaOnce(
    Message message, {
    required bool isGroup,
  }) {
    final key = _incomingMediaKey(message.peerId, message.id);
    final activeRestore = _incomingMediaActiveRestores[key];
    if (activeRestore != null) {
      return activeRestore;
    }

    _relayMediaRetry.cancelTimerForKey(key);
    late final Future<String?> restoreFuture;
    restoreFuture =
        (isGroup
                ? _restoreGroupBlobMediaFromRelay(message)
                : _restoreDirectBlobMediaFromRelay(message))
            ...
            .whenComplete(() {
              if (identical(_incomingMediaActiveRestores[key], restoreFuture)) {
                _incomingMediaActiveRestores.remove(key);
              }
            });
    _incomingMediaActiveRestores[key] = restoreFuture;
  ...
  void _restoreMediaInBackground(
    ...
    if (force) {
      if (!_isIncomingRelayMediaPlaceholder(message)) {
        return;
      }
    ...
    final key = _incomingMediaKey(message.peerId, message.id);
    if (_incomingMediaActiveRestores.containsKey(key)) {
      return;
    }
  ...
  bool isIncomingRelayMediaRestoreInProgress(Message message) {
    ...
    if (_isIncomingRelayMediaBusyStatus(status)) {
      return true;
    }
    return false;
  }
  ...
  bool isIncomingRelayMediaRestoreFailed(Message message) {
    return _isIncomingRelayMediaPlaceholder(message) &&
        (message.transferStatus ?? '').trim() == _incomingRelayErrorStatus;
  }
  ...
  bool _isFailedIncomingMediaPlaceholder(Message message) {
    ...
    return (message.transferStatus ?? '').trim() == _incomingRelayErrorStatus;
  }
  ...
  Future<String?> _restoreMediaFromRelay({
    ...
    try {
      final result = await _relayMediaTransfer.restoreIncomingMedia(
        peerId: peerId,
        messageId: messageId,
        blobId: blobId,
        fileName: fileName,
      ...
      await _relayMediaRetry.clear(peerId, messageId);
      _messageUpdatesController.add(peerId);
      return path;
    } catch (error) {
      AppFileLogger.log(
  ...
  Future<void> _markIncomingRelayRestoreFailed({
    ...
    var canRetry = false;
    try {
      canRetry = await _relayMediaRetry.recordFailure(
        peerId: peerId,
        messageId: messageId,
        errorKind: errorKind,
      );
    ...
    try {
      failedMessage = await _findMessage(peerId, messageId);
    } catch (findError) {
      AppFileLogger.log(
    ...
    if (failedMessage != null &&
      ...
      _relayMediaRetry.scheduleRetry(
        ...
        restoreInBackground: (message, {required bool isGroup}) {
          _restoreMediaInBackground(message, isGroup: isGroup, force: true);
        },
      );
    } else {
    ...
    if (!_messageUpdatesController.isClosed) {
      _messageUpdatesController.add(peerId);
    }
  }
  ...
  bool _shouldAutoRestoreIncomingMedia(Message message) {
    ...
    if (status == _incomingRelayErrorStatus) {
      return _relayMediaRetry.isDue(
        _incomingMediaKey(message.peerId, message.id),
      );
    }
    return false;
  }
  ...
  bool _shouldResumeIncomingMedia(Message message) {
    ...
    final status = (message.transferStatus ?? '').trim();
    if (_isIncomingRelayMediaBusyStatus(status) ||
        status == _incomingRelayErrorStatus) {
      return _relayMediaRetry.isDue(key);
    }
    return status.isEmpty;
  }

  bool _isIncomingRelayMediaBusyStatus(String status) {
  ...
  bool _isStaleIncomingRelayProgress({
    ...
    if (currentRank == nextRank &&
        currentRank ==
            _incomingRelayMediaStatusRank(_incomingRelayDownloadStatus) &&
  ...
  int _incomingRelayMediaStatusRank(String? status) {
    ...
    if (normalized == _incomingRelayFetchStatus ||
        normalized == _incomingRelayRetryStatus) {
      return 1;
    }
    if (normalized == _incomingRelayDownloadStatus) {
      return 2;
    }
    if (normalized == _incomingRelayCompleteStatus) {
      return 3;
    }
    if (normalized == _incomingRelayDecryptStatus) {
      return 4;
    }
    if (normalized == _incomingRelaySaveStatus) {
      return 5;
    }
    return 0;
  }

  bool _isIncomingRelayMediaPlaceholder(Message message) {
    if (!message.incoming || message.kind != MessageKind.file) {
      return false;
    }
  ...
  Future<void> dispose() async {
    ...
    final pendingUpdates = List<_PendingProgressUpdate>.from(
    ...
    _incomingMediaActiveRestores.clear();
...
Future<Uint8List?> _encryptGroupBytesIsolate({
  ...
  final totalLength =
      _groupMediaCipherMagicV2.length +
      1 +
      1 +
      2 +
      groupIdBytes.length +
...
Future<Uint8List?> _decryptGroupBytesIsolate({
  ...
  if (encryptedBytes.length >=
      _groupMediaCipherMagicV2.length + 1 + 1 + 2 + 1) {
    var magicMatch = true;
