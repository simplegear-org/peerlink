...
class ChatControllerMedia {
  ...
  static Future<void> processLoadedMessages({
    ...
    for (var i = 0; i < stored.length; i++) {
      ...
      if (message.localFilePath != null && message.localFilePath!.isNotEmpty) {
        ...
        if (message.fileDataBase64?.isNotEmpty == true) {
          continue;
        }
  ...
  static Future<void> replaceCompletedFileMessage({
    ...
    final thumbnailPath = path == null || path.isEmpty
        ? null
        : await ensureThumbnail(
            ChatMessageCopy.copy(existing, localFilePath: path),
          );
    ...
    try {
      ...
      if (path.isNotEmpty) {
        final thumbnailPath = await ensureThumbnail(
          ChatMessageCopy.copy(message, localFilePath: path),
        );
        await replaceMessage(
          peerId,
          message.id,
