...
class ChatControllerMedia {
  static Future<void> processLoadedMessages({
    ...
    required Future<void> Function(Message message) deleteManagedMediaForMessage,
  }) async {
    developer.log(
    ...
    for (var i = 0; i < stored.length; i++) {
      ...
      if (message.localFilePath != null && message.localFilePath!.isNotEmpty) {
          ...
          if (!storage.isManagedMediaPath(message.localFilePath)) {
            try {
              ...
              if (path.isNotEmpty && path != message.localFilePath) {
                final updated = ChatMessageCopy.copy(message, localFilePath: path);
                stored[i] = updated;
        ...
        if (recoveredPath != null && recoveredPath.isNotEmpty) {
          final updated = ChatMessageCopy.copy(message, localFilePath: recoveredPath);
          stored[i] = updated;
          changedMessages[updated.id] = updated;
          changed = true;
          recoveredCount += 1;
        ...
        if (message.fileDataBase64 != null && message.fileDataBase64!.isNotEmpty) {
          try {
    ...
    if (changed) {
      ...
      if (changedMessages.isNotEmpty) {
        await upsertStoredMessages(peerId, changedMessages.values.toList(growable: false));
  ...
  static Future<void> replaceCompletedFileMessage({
    ...
    try {
      ...
      developer.log('[chat] Failed to save media bytes: $e\n$stack', name: 'chat');
      path = null;
    }
