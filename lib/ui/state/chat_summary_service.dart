...
class ChatSummaryService {
  ...
  Set<String> knownGroupIds() {
    return _groupMetaByGroupId.entries
        .where((entry) => entry.value['isGroup'] == true)
        .map((entry) => entry.key.trim())
        .where((groupId) => groupId.isNotEmpty)
        .toSet();
  }

  Chat? knownGroupChat(String groupId) {
    final meta = _groupMetaByGroupId[groupId];
    if (meta == null || meta['isGroup'] != true) {
      return null;
    }
