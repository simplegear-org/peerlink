...
class ChatGroupFlowService {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      try {
        await facade.sendDirectPushEvent(
          directPeerId: peerId,
          messageId:
              'group-key:${groupChat.peerId}:${DateTime.now().microsecondsSinceEpoch}:$i',
          notificationType: 'group_key',
          data: <String, dynamic>{
            'type': 'direct_update',
            'directPeerId': peerId,
        ...
        // Push is only a wake-up hint; relay control remains authoritative.
      }
    }
  }
  ...
  Future<void> sendGroupInvites(
    ...
    for (var i = 0; i < targetRecipients.length; i++) {
      ...
      final messageId = 'invite:${groupChat.peerId}:${nextLocalMessageId()}:$i';
      try {
        developer.log(
  ...
  Future<void> broadcastGroupMembersUpdate({
    ...
    final targets = recipients
        ...
        .toSet()
        .toList(growable: false);
    final pushPayload = outboundCodec.decodeGroupMembersPayloadForPush(payload);
