...
import 'core/firebase/firebase_push_servers_merge_orchestrator.dart';
...
import 'core/runtime/push_servers_service.dart';
import 'core/runtime/relay_servers_service.dart';
import 'core/runtime/turn_servers_service.dart';
...
class _BootstrapAppState extends State<_BootstrapApp> {
  ...
  String? _registeredRelayFcmToken;
    ...
    try {
      ...
      if (firebaseReady && _shouldInitializeFcm()) {
        ...
        try {
          ...
          await firebaseMessagingService.initialize().timeout(
            _fcmInitTimeout,
            onTimeout: () {
              throw TimeoutException(
                'FCM initialization timed out after ${_fcmInitTimeout.inSeconds}s',
              );
            },
          );
      ...
      _globalNodeFacade = deps.nodeFacade;
      FirebaseMessagingService.onServersFromPush = (update) async {
        await _applyPushServersToRuntime(
          ...
          push: update.push,
          turn: update.turn,
          priorityBootstrap: update.priorityBootstrap,
          priorityRelay: update.priorityRelay,
          priorityPush: update.priorityPush,
          priorityTurn: update.priorityTurn,
        );
      };
      FirebaseMessagingService.onAccountMembershipUpdateFromPush = (payload) {
        return deps.nodeFacade.applyAccountMembershipUpdate(
      ...
      await _applyServersFromStorageToRuntime(
        facade: deps.nodeFacade,
        storage: storage,
      );
      final iosCallkitService = IosCallkitService.instance;
      _iosCallkitService = iosCallkitService;
      unawaited(
        iosCallkitService.initialize(
          deps.nodeFacade,
          onPushPayload: (payload, {required source}) async {
            const FirebasePushServersMergeOrchestrator().scheduleIfPresent(
              payload,
              source: source,
              logName: 'callkit',
              logPrefix: '[callkit][servers]',
            );
          },
        ),
      );
  ...
  Future<void> _syncFcmTokenWithRuntime(
    NodeFacade facade,
    String? token, {
    String? previousToken,
  }) async {
    final normalizedToken = token?.trim();
    final normalizedPrevious = previousToken?.trim();
    AppFileLogger.log(
      '[main] syncFcmToken start hasToken=${normalizedToken != null && normalizedToken.isNotEmpty} '
      'hasPrevious=${normalizedPrevious != null && normalizedPrevious.isNotEmpty}',
    );

    try {
      await facade.updateFcmToken(normalizedToken);
    } catch (error, stackTrace) {
      AppFileLogger.log(
        '[main] updateFcmToken failed error=$error',
        stackTrace: stackTrace,
      );
    }
    ...
    if (normalizedPrevious != null &&
        ...
        normalizedPrevious != normalizedToken) {
      try {
        await facade.unregisterPushDeviceToken(normalizedPrevious);
      } catch (error, stackTrace) {
        AppFileLogger.log(
          '[main] unregisterPushDeviceToken failed error=$error',
          stackTrace: stackTrace,
        );
      }
    }
    ...
    if (normalizedToken == null || normalizedToken.isEmpty) {
      AppFileLogger.log('[main] syncFcmToken skip register: empty token');
      _registeredRelayFcmToken = null;
      return;
    }
    ...
    try {
      ...
      AppFileLogger.log(
        '[main] registerPushDeviceToken failed error=$error',
        stackTrace: stackTrace,
      );
    }
  }
  ...
  void dispose() {
    ...
    if (iosCallkitService != null) {
      unawaited(iosCallkitService.dispose());
    }
...
Future<void> _applyServersFromStorageToRuntime({
  required NodeFacade facade,
  required StorageService storage,
}) async {
  final settings = storage.getSettings();
  final bootstrap =
      (settings.get('bootstrap_servers') as List? ?? const <dynamic>[])
          .map(
            (item) =>
  ...
  final relay = (settings.get('relay_servers') as List? ?? const <dynamic>[])
      .map((item) => RelayServersService.normalizeEndpoint(item.toString()))
      .where((item) => item.isNotEmpty)
      .toSet()
      .toList(growable: false);
  final push = PushServersService.extractActiveEndpointsFromStorage(
    settings.get('push_servers'),
  );
  final turn = (settings.get('turn_servers') as List? ?? const <dynamic>[])
      .whereType<Map>()
      ...
      .toList(growable: false);
  AppFileLogger.log(
    '[main][servers] reconcile from storage bootstrap=${bootstrap.length} relay=${relay.length} push=${push.length} turn=${turn.length}',
  );
  await _applyPushServersToRuntime(
    facade: facade,
    storage: storage,
    ...
    push: push,
    turn: turn,
    priorityBootstrap: const <String>[],
...
Future<void> _applyPushServersToRuntime({
  required NodeFacade facade,
  required StorageService storage,
  required List<String> bootstrap,
  required List<String> relay,
  required List<String> push,
  required List<TurnServerConfig> turn,
  required List<String> priorityBootstrap,
  ...
  if (bootstrap.isEmpty &&
    ...
    AppFileLogger.log('[main][servers] apply skipped: empty update');
    return;
  }
  AppFileLogger.log(
    '[main][servers] apply start bootstrap=${bootstrap.length} relay=${relay.length} push=${push.length} turn=${turn.length}',
  );
  final health = ServerHealthCoordinator(facade: facade, storage: storage);
  await health.initialize();
  if (bootstrap.isNotEmpty) {
    await health.bootstrap.merge(bootstrap);
  }
  if (relay.isNotEmpty) {
    await health.relay.merge(relay);
  }
  if (priorityRelay.isNotEmpty) {
    await health.relay.merge(priorityRelay);
  }
  if (push.isNotEmpty) {
    await health.push.merge(push);
  }
  ...
  if (priorityBootstrap.isNotEmpty) {
    for (final endpoint in priorityBootstrap.reversed) {
      await health.bootstrap.putFirst(endpoint);
    }
  }
