...
class GroupMessageCryptoService {
  ...
  Future<Uint8List?> encryptGroupBytes({
    ...
    if (plainBytes.length >= 256 * 1024) {
      final plainData = TransferableTypedData.fromList(<Uint8List>[plainBytes]);
      final encryptedData = await Isolate.run(() async {
        final encrypted = await _encryptGroupBytesIsolate(
          groupId: groupId,
          keyBase64: keyBase64,
          plainBytes: plainData.materialize().asUint8List(),
        );
        return encrypted == null
            ? null
            : TransferableTypedData.fromList(<Uint8List>[encrypted]);
      });
      return encryptedData?.materialize().asUint8List();
    }
  ...
  Future<Uint8List?> decryptGroupBytes({
    ...
    if (encryptedBytes.length >= 256 * 1024) {
      final encryptedData = TransferableTypedData.fromList(<Uint8List>[
        encryptedBytes,
      ]);
      final decryptedData = await Isolate.run(() async {
        final decrypted = await _decryptGroupBytesIsolate(
          groupId: groupId,
          keyBase64: keyBase64,
          encryptedBytes: encryptedData.materialize().asUint8List(),
        );
        return decrypted == null
            ? null
            : TransferableTypedData.fromList(<Uint8List>[decrypted]);
      });
      return decryptedData?.materialize().asUint8List();
    }
