...
class SessionManager {
  ...
  final Map<String, CryptoPeerSession> _sessions = {};
  ...
  CryptoPeerSession? getSession(String peerId) {
  ...
  Future<void> trustPeerIdentity({
    ...
    if (existing != null) {
      if (_sameBytes(existing.signingPublicKey.bytes, signingPublicKey.bytes) &&
          _sameBytes(
            existing.agreementPublicKey.bytes,
  ...
  Future<bool> trustPeerIdentityBundleV3(
    ...
    final bundle = PeerIdentityBundleV3.fromJson(json);
    if (bundle == null) {
      return false;
    }
    final normalizedExpected = expectedPeerId?.trim();
    if (normalizedExpected != null &&
        normalizedExpected.isNotEmpty &&
        bundle.peerId != normalizedExpected) {
      return false;
    }
    try {
      final signingBytes = base64Decode(bundle.signingPublicKey);
      final agreementBytes = base64Decode(bundle.agreementPublicKey);
      final signingPublicKey = SimplePublicKey(
        signingBytes,
        type: KeyPairType.ed25519,
      );
      final verified = await signatures.verify(
        PeerIdentityBundleV3.signaturePayload(
          peerId: bundle.peerId,
          signingPublicKey: bundle.signingPublicKey,
          agreementPublicKey: bundle.agreementPublicKey,
        ),
        Uint8List.fromList(base64Decode(bundle.signature)),
        signingPublicKey,
      );
      if (!verified) {
        return false;
      }
      await trustPeerIdentity(
        peerId: bundle.peerId,
        signingPublicKey: signingPublicKey,
  ...
  Future<Uint8List> receiveHandshake(String peerId, Uint8List payload) async {
    ...
    final session = CryptoPeerSession(
      peerId: peerId,
