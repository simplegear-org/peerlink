...
class BootstrapSignalingService implements SignalingService {
  ...
  Future<void> setServer(String endpoint) async {
    ...
    if (!_isSafeBootstrapUri(uri)) {
      _setError('invalid endpoint: некорректный адрес');
      _setStatus(SignalingConnectionStatus.error);
      _log('connect:invalid endpoint=$canonicalEndpoint error=unsafe host/scheme');
      return;
    }
  ...
  WebSocketChannel _connectWebSocket(Uri uri) {
    try {
      ...
      throw StateError('invalid bootstrap endpoint: ${uri.toString()} ($error)');
    }
  }
