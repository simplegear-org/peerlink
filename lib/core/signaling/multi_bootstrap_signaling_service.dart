...
class MultiBootstrapSignalingService implements SignalingService {
  ...
  List<String> get connectedEndpoints => _configuredEndpoints
      .where(
        (endpoint) =>
            _statusByEndpoint[endpoint] == SignalingConnectionStatus.connected,
      )
  ...
  List<String> targetEndpointsForPeer(String peerId) {
    return _targetServicesForPeer(peerId).keys.toList(growable: false);
  }

  List<String> connectedTargetEndpointsForPeer(String peerId) {
  ...
  Future<void> configureServers(List<String> endpoints) async {
    ...
    if (_sameConfiguredEndpoints(normalized)) {
      _log(
        'configure:skip unchanged instance=$instanceId endpoints=${normalized.join(",")}',
      );
      return;
    }
    _log(
      'configure:apply instance=$instanceId endpoints=${normalized.join(",")}',
    );
    ...
    for (final endpoint in normalized) {
      ...
      unawaited(_connectEndpoint(service, endpoint));
    }
    _emitStatus();
    _emitError();
  }
  ...
  Future<void> _connectEndpoint(
    BootstrapSignalingService service,
    String endpoint,
  ) async {
    try {
      await service.setServer(endpoint);
    } catch (error) {
      _log('configure:error endpoint=$endpoint error=$error');
    }
  }

  bool _sameConfiguredEndpoints(List<String> normalized) {
    if (_configuredEndpoints.length != normalized.length) {
      return false;
    }
    final current = _configuredEndpoints.toSet();
    final next = normalized.toSet();
    if (current.length != next.length) {
      return false;
    }
    return current.containsAll(next);
  }
  ...
  Future<void> _ensureEndpoint(String endpoint) async {
    if (_services.containsKey(endpoint)) {
      _log('ensureEndpoint:reuse instance=$instanceId endpoint=$endpoint');
      return;
    }
    ...
    _log(
      'ensureEndpoint:new instance=$instanceId endpoint=$endpoint bootstrapInstance=${service.instanceId}',
    );
    ...
    _peerSubs[endpoint] = service.peersStream.listen((peers) {
  ...
  void _emitStatus() {
    ...
    _log(
      'status=$next connected=${connectedEndpoints.length}/${_configuredEndpoints.length}',
    );
  }
  ...
  bool _shouldLogSignalType(String type) {
    switch (type) {
      case 'ice':
  ...
  void _log(String message) {
    AppFileLogger.log('[multi_bootstrap][$_selfPeerId][${_logSeq++}] $message');
  }
}
