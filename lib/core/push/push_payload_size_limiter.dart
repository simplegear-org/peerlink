...
class PushPayloadSizeLimiter {
  ...
  Map<String, dynamic> compact(Map<String, dynamic> payload) {
    ...
    const serverListPaths = <_ServerListPath>[
    ...
    for (final path in serverListPaths) {
      _trimServerListToFirst(result, path);
    }
    if (_transportSizeBytes(result) <= maxTransportPayloadBytes) {
      return result;
    }
    ...
    for (final path in serverListPaths) {
      while (_transportSizeBytes(result) > maxTransportPayloadBytes) {
  ...
  bool _trimServerListToFirst(
    ...
    sectionMap[path.key] = <dynamic>[list.first];
    payload[path.section] = sectionMap;
    return true;
  }
