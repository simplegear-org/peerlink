...
class TurnAllocator {
  ...
  void configureServers(List<TurnServerConfig> servers) {
    final validServers = <TurnServerConfig>[];
    for (final server in servers) {
      final url = server.url.trim();
      if (url.isEmpty) {
        continue;
      }
      if (!_isSafeTurnUrl(url)) {
        _log('configure:skip invalid url=$url');
        continue;
      }
      validServers.add(server.copyWith(url: url));
    }
    _nodes
      ..clear()
      ..addAll(
  ...
  bool _isSafeTurnUrl(String raw) {
  ...
  List<TurnCredentials> allocateAll() {
    ...
    final ordered = List<TurnNode>.from(_nodes)..sort(_compareNodes);
    ...
    return source.map((node) => node.credentials).toList(growable: false);
  }
  ...
  Future<void> refreshSelectionIfNeeded({
    ...
    final shortlist = List<TurnNode>.from(_nodes)..sort(_compareNodes);
  ...
  void reportSuccess(String url) {
    _findNode(url)?.markHealthy();
    _onSuccess?.call(url);
    _recalculateActive();
  }

  void reportFailure(String url) {
    _findNode(url)?.markUnhealthy();
    _onFailure?.call(url);
    _recalculateActive();
  }
  ...
  List<TurnServerConfig> get orderedServerConfigs {
    if (_nodes.isEmpty) {
      return const <TurnServerConfig>[];
    }
    final ordered = List<TurnNode>.from(_nodes)..sort(_compareNodes);
    final healthy = ordered
        .where((node) => _effectiveHealthy(node) == true)
  ...
  List<TurnServerConfig> get healthyOrderedServerConfigs {
    if (_nodes.isEmpty) {
      return const <TurnServerConfig>[];
    }
    final ordered = List<TurnNode>.from(_nodes)..sort(_compareNodes);
  ...
  TurnNode? _findNode(String url) {
    try {
      return _nodes.firstWhere((n) => n.credentials.url == url);
    } catch (_) {
      return null;
    }
  }
  ...
  bool? _effectiveHealthy(TurnNode node) {
    final lookup = _availabilityLookup;
    if (lookup == null) {
      return node.isHealthy;
    }
    final shared = lookup(node.credentials.url);
    return shared?.isAvailable ?? node.isHealthy;
  }
