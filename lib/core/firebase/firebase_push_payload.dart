...
class FirebasePushPayload {
  ...
  factory FirebasePushPayload.fromMap(Map<String, dynamic> data) {
    ...
    return FirebasePushPayload(
      ...
      type: (_readString(data, nestedData, const <String>['type']) ?? '')
          .toLowerCase(),
      senderPeerId:
          _readString(data, nestedData, const <String>[
            'fromPeerId',
            'peerId',
            'callerUserId',
            'senderUserId',
          ...
          '',
      callId:
          _readString(data, nestedData, const <String>['callId', 'call_id']) ??
          '',
      callAction:
          (_readString(data, nestedData, const <String>[
                    'callAction',
      ...
      mediaType:
          (_readString(data, nestedData, const <String>[
              ...
              .toLowerCase(),
      relayMessageId:
          _readString(data, nestedData, const <String>['relayMessageId']) ??
          _readNestedString(
            _readValue(data, nestedData, const <String>['relay']),
            'relayMessageId',
          ) ??
          '',
      groupId: _readString(data, nestedData, const <String>['groupId']) ?? '',
      directPeerId:
          _readString(data, nestedData, const <String>['directPeerId']) ?? '',
      notificationText:
          _readString(data, nestedData, const <String>['message', 'text']) ??
          '',
    );
  }
  ...
  List<String> get relayServers {
    final relay = _decodeNestedMap(rawRelay);
    if (relay == null) {
      return const <String>[];
    }
    final servers = relay['servers'];
    if (servers is! List) {
      return const <String>[];
    }
    return servers
  ...
  List<String> get availableRelayServers {
    final raw = rawServers;
    final servers = _decodeNestedMap(raw);
    final relay = servers?['relay'];
    if (relay is! List) {
      return const <String>[];
    }
    return relay
  ...
  static Map<String, dynamic>? _extractNestedData(Map<String, dynamic> data) {
    return _decodeNestedMap(data['data']) ?? _decodeNestedMap(data['payload']);
  }

  static Map<String, dynamic>? _decodeNestedMap(Object? raw) {
    if (raw is Map) {
  ...
  static String? _readNestedString(Object? raw, String key) {
    final nested = _decodeNestedMap(raw);
