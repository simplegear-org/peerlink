...
class ChatSendReceipt {
  ...
  const ChatSendReceipt({required this.sent, required this.relayServers});

  static const empty = ChatSendReceipt(sent: false, relayServers: <String>[]);
}
...
class ChatService {
  ...
  Future<ChatSendReceipt> sendPayload(
    ...
    final targetLabel = targetKind == ChatPayloadTargetKind.group
        ? 'group'
        : 'peer';
    ...
    try {
      ...
      final servers = _serverMetadataProvider?.call();
      if (servers != null && servers.isNotEmpty) {
        payload['servers'] = servers;
      }
      final receipt = await _messaging.sendPayload(
        targetId,
        payload,
  ...
  Future<Uint8List> encryptDirectBytes(String peerId, Uint8List plainBytes) {
    return _messaging.encryptForPeer(peerId, plainBytes);
  }
  ...
  Future<Uint8List> decryptDirectBytes(
    ...
    return _messaging.decryptFromPeer(peerId, encryptedBytes);
  }
  ...
  Future<void> discardPendingPayload({
    ...
    return _messaging.discardPendingPayload(
      targetKind: targetKind == ChatPayloadTargetKind.group
          ? MessagingTargetKind.group
          : MessagingTargetKind.direct,
      targetId: targetId,
  ...
  Future<void> sendControlMessage(
    ...
    _log(
      'send target=peer:$peerId kind=$kind text=${text.length} control=true',
    );
    try {
      await sendPayload(
        peerId,
  ...
  Future<void> sendFile(
    ...
    if (fileBytes != null) {
      ...
      resolvedBytes = await readFileBytesInIsolate(filePath!);
    }
  ...
  Future<bool> _handleIncoming(ReliableMessageEnvelope envelope) async {
    final payload = envelope.payload;
    final sourcePeerId = envelope.fromPeerId;
    final groupId = envelope.groupId?.trim();
    ...
    _log(
      groupId != null && groupId.isNotEmpty
          ? 'recv target=$groupId from=$sourcePeerId id=$id'
          : 'recv target=peer:$sourcePeerId id=$id',
    );
