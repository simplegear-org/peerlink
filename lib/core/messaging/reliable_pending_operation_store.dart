...
class ReliablePendingOperation {
  ...
  String diagnosticsSummary({int? nowMs}) {
    final dueInMs = nextAttemptMs <= 0
        ? 0
        : nextAttemptMs - (nowMs ?? DateTime.now().millisecondsSinceEpoch);
    return 'id=$operationId kind=${kind.name} '
        'target=${targetId ?? groupId ?? ""} messageId=${messageId ?? ""} '
        'forcePlain=$forcePlain attempts=$attempts '
        'nextAttemptMs=$nextAttemptMs dueInMs=$dueInMs '
        'recipients=${recipients?.length ?? memberPeerIds?.length ?? 0}';
  }
}
...
class ReliablePendingOperationStore {
  ...
  Future<void> restore() async {
    ...
    if (_operations.isNotEmpty) {
      ...
      for (final operation in _operations.values) {
        _log(
          'pending:restored item ${operation.diagnosticsSummary(nowMs: nowMs)}',
        );
      }
    }
  }
  ...
  Future<void> upsert(ReliablePendingOperation operation) async {
    _operations[operation.operationId] = operation;
    _log('pending:upsert ${operation.diagnosticsSummary()}');
    await persist();
  }
