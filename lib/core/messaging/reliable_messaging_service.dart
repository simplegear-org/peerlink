...
class ReliableMessagingService {
  static const Duration _prekeyBundleTtl = Duration(days: 30);
  static const String _prekeyBundleMimeType = 'application/vnd.peerlink.prekey+json';
  ...
  Future<void> sendPayload(
    ...
    if (targetKind == MessagingTargetKind.group) {
      final normalizedRecipients = (recipients ?? const <String>[])
          .map((item) => item.trim())
  ...
  Future<void> updateGroupMembers({
    ...
    final normalizedMembers = memberPeerIds
        .map((item) => item.trim())
        .where((item) => item.isNotEmpty)
        .toSet()
        .toList(growable: false)
  ...
  Future<String> storeBlob({
    ...
    if (!_messageRelayEnabled) {
      _log('blob store failed scope=$scopeKind target=$targetId reason=relay not configured');
      throw StateError('No message relay server configured');
    }
  ...
  Future<void> _pollRelay() async {
    ...
    _pollInFlight = true;
    try {
      _log('messageRelay:poll start selfId=$_selfId cursor=$_fetchCursor');
      ...
      _log('messageRelay:poll received messages=${result.messages.length} cursor=${result.cursor}');
      _fetchCursor = result.cursor;
      
      int processedCount = 0;
      for (final envelope in result.messages) {
        _log('messageRelay:poll processing envelope id=${envelope.id} from=${envelope.from} to=${envelope.to}');
        await _handleRelayEnvelope(envelope);
        processedCount++;
      }
      
      _log('messageRelay:poll complete processed=$processedCount');
    } catch (e, stack) {
  ...
  Future<void> _handleRelayEnvelope(RelayEnvelope envelope) async {
    ...
    if (!shouldAck) {
      _log('messageRelay:drop acknowledged id=${envelope.id}');
    }
  ...
  Future<bool> _handleReliableEnvelope({
    ...
    switch (type) {
      case _EnvelopeType.plainMessage:
        if (!_validateReplayWindow(id, ts)) {
          return false;
        }
        await _handlePlainMessage(
          id: id,
  ...
  Future<void> _handleSecureMessage({
    ...
    if (!await _ensureSessionForPeer(fromPeerId)) {
      ...
      _log('recv:secure queued id=$id peer=$fromPeerId reason=session not established');
  ...
  Future<void> _handlePlainMessage({
    required String id,
    required String fromPeerId,
    required int timestampMs,
  ...
  bool _validateReplayWindow(String id, int timestampMs) {
    final now = DateTime.now().millisecondsSinceEpoch;
    final futureDelta = timestampMs - now;

    if (futureDelta > _maxFutureClockSkew.inMilliseconds) {
      _log(
        'recv:drop timestamp too far in future id=$id deltaMs=$futureDelta',
      );
      return false;
    }
  ...
  Future<void> _publishPrekeyBundle() async {
    if (!_messageRelayEnabled) {
      return;
    }
    try {
      ...
      await _relay.storeBlob(
        RelayBlobUploadEnvelope(
          id: _prekeyBundleId(_selfId),
          from: _selfId,
          groupId: _selfId,
          fileName: 'prekey-bundle.json',
          mimeType: _prekeyBundleMimeType,
          timestampMs: timestampMs,
          ttlSeconds: _prekeyBundleTtl.inSeconds,
          payload: bytes,
          signature: signature,
          senderSigningPublicKey: signingPublicKey,
        ),
      );
      _log('prekey:publish ok id=${_prekeyBundleId(_selfId)}');
    } catch (e) {
      _log('prekey:publish error=$e');
    }
  }
