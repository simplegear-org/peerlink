...
class RelayHttpMessageApi {
  ...
  Future<RelayWriteReceipt> postToQuorum(
    ...
    if (operationName == 'group-store') {
      log(
        'relay group-store failed success=$successCount/${targets.length} '
        'required=${effectiveQuorum(targets.length, quorum)} '
        'errors=${errors.join(' | ')}',
      );
    }
  ...
  Future<RelayFetchResult> fetchFromServers(
    ...
    if (targets.isEmpty) {
      log(
        'fetch-targeted skip recipient=$recipientId reason=no valid relay servers',
      );
      return RelayFetchResult(
        messages: const [],
        cursor: cursor,
  ...
  Future<RelayFetchOutcome> fetchFromServer(
    ...
    try {
      ...
      final groupCount = messages
          ...
          .length;
      if (messages.isNotEmpty || groupCount > 0) {
        log(
          'fetch ok server=${server.toString()} recipient=$recipientId '
          'messages=${messages.length} groupMessages=$groupCount cursor=$nextCursor',
        );
      }
  ...
  Future<RelayPostOutcome> postToServer(
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        if (operationName == 'group-store') {
          log(
            'relay group-store ok server=${server.toString()} '
      ...
      if (operationName == 'group-store') {
        log(
          'relay group-store rejected server=${server.toString()} '
          'status=${response.statusCode} body=${response.body}',
        );
      }
