...
class HttpRelayClient implements RelayClient {
  ...
  _refreshAvailabilityForEndpoints;
  int _cursorIndex = 0;
  ...
  Future<bool> _storeBlobChunked(
    ...
    _log(
      ...
      'chunkSize=$_blobChunkSizeBytes servers=${targets.length}',
    );
    for (final server in targets) {
      ...
      try {
        ...
        final workerCount = totalChunks < _blobChunkUploadConcurrency
            ? totalChunks
            : _blobChunkUploadConcurrency;
        await Future.wait(
          List<Future<void>>.generate(workerCount, (_) => uploadWorker()),
        );

        if (firstError != null) {
          _markUnhealthy(server, 'blob-upload chunk failed');
          throw firstError!;
        }
        if (endpointMissing) {
          _log(
            'blob-upload chunk endpoint missing server=${server.toString()}',
          );
          continue;
        }
  ...
  Future<RelayBlobDownload> fetchBlob(
    ...
    final errors = <String>[];
    final initialTargets = await _liveServers(limit: _maxActiveRelayPool);
    final initialBatch = _collectBlobFetchBatch(
      attempted,
      errors,
      await _fetchBlobAcrossServers(
        initialTargets,
        blobId,
        onProgress: onProgress,
      ),
    ...
    if (allInitialNotFound && attempted.length < _servers.length) {
      final fallbackTargets = _prioritizedServers(limit: _servers.length)
          .where((server) => !attempted.contains(server.toString()))
          .toList(growable: false);
      final fallbackBatch = _collectBlobFetchBatch(
        attempted,
        errors,
        await _fetchBlobAcrossServers(
          fallbackTargets,
    ...
    if (notFoundErrors.isNotEmpty && notFoundErrors.length == errors.length) {
      _log(
        'blob-fetch blobId=$blobId not-found attempted=${attempted.length}/${_servers.length}',
      );
  ...
  Future<List<_BlobFetchServerOutcome>> _fetchBlobAcrossServers(
    List<Uri> servers,
    String blobId, {
    void Function({
      required int receivedBytes,
    ...
    return Future.wait(
      servers.map(
        (server) async => _BlobFetchServerOutcome(
          server: server,
          outcome: await _fetchBlobFromServer(
            server,
            blobId,
            onProgress: onProgress,
          ),
  ...
  _BlobFetchBatchOutcome _collectBlobFetchBatch(
    Set<String> attempted,
    List<String> errors,
    List<_BlobFetchServerOutcome> outcomes,
  ) {
    if (outcomes.isEmpty) {
      return const _BlobFetchBatchOutcome();
    }
    ...
    for (final result in outcomes) {
      ...
      if (download != null) {
        return _BlobFetchBatchOutcome(download: download);
      }
    ...
    return _BlobFetchBatchOutcome(errors: localErrors);
  }
  ...
  Future<RelayFetchResult> fetch(
    ...
    if (_servers.isEmpty) {
      _log('fetch skip recipient=$recipientId reason=no relay servers');
    ...
    final outcomes = await Future.wait(
      (await _fetchTargets()).map(
        (server) =>
            _fetchFromServer(server, recipientId, cursor: cursor, limit: limit),
      ),
    );
  ...
  void configureServers(List<String> servers) {
    ...
    for (final raw in servers) {
      ...
      final normalized = _normalizeBase(trimmed, httpsOnly: _httpsOnly);
      if (normalized == null) {
        _log('configure:skip invalid relay endpoint=$trimmed');
        continue;
      }
  ...
  List<Uri> _candidateServers({required int limit}) {
    ...
    final rotated = _prioritizedServers(limit: _servers.length);
  ...
  List<Uri> _prioritizedServers({required int limit}) {
    if (_servers.isEmpty) {
      return const <Uri>[];
    }
  ...
  int _compareServers(Uri a, Uri b) {
    ...
    final aHealthy = _sortRankForAvailability(
      aAvailability,
      _statuses[a.toString()],
    );
    final bHealthy = _sortRankForAvailability(
      bAvailability,
      _statuses[b.toString()],
    );
  ...
  Future<http.Response?> _sendGet(
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        ...
        final streamedResult = await _awaitWithTimeoutResult(
        ...
        if (streamedResult.hasError) {
          ...
          final transient = _isTransientHttpError(streamedResult.error);
        ...
        lastError = e;
        final transient = _isTransientHttpError(e);
        if (!transient || attempt >= 2) {
          break;
        }
    ...
    if (lastError != null && _isTransientHttpError(lastError)) {
      return null;
    }
  ...
  Future<_AwaitResult<T>> _awaitWithTimeoutResult<T>(
    Future<T> future,
    Duration timeout,
  ) async {
    const timeoutMarker = Object();
    ...
    if (identical(result, timeoutMarker)) {
      return _AwaitResult<T>.timedOut();
    }
    if (result is _AsyncErrorBox) {
      return _AwaitResult<T>.error(result.error, result.stackTrace);
    }
    return _AwaitResult<T>.value(result as T);
  }
  ...
  static Uri? _normalizeBase(String raw, {required bool httpsOnly}) {
    ...
    if (!_isSafeRelayHost(uri.host.trim())) {
      return null;
    }
  ...
  static bool _isSafeRelayHost(String host) {
    final value = host.trim();
    if (value.isEmpty || value.contains('%')) {
      return false;
    }
    if (_isStaticIpAddressHost(value)) {
      return true;
    }
  ...
  static bool _isStaticIpAddressHost(String host) {
    if (host.isEmpty) {
      return false;
    }
    try {
...
class _AwaitResult<T> {
  final T? value;
  final Object? error;
  final StackTrace? stackTrace;
  final bool timedOut;

  const _AwaitResult.value(T this.value)
    : error = null,
      stackTrace = null,
      timedOut = false;

  const _AwaitResult.error(Object this.error, StackTrace this.stackTrace)
    : value = null,
      timedOut = false;

  const _AwaitResult.timedOut()
    : value = null,
      error = null,
      stackTrace = null,
      timedOut = true;
...
class _BlobFetchServerOutcome {
  final Uri server;
  final _BlobFetchOutcome outcome;

  const _BlobFetchServerOutcome({required this.server, required this.outcome});
}

class _BlobFetchBatchOutcome {
  final RelayBlobDownload? download;
  final List<String> errors;

  const _BlobFetchBatchOutcome({this.download, this.errors = const <String>[]});
}
