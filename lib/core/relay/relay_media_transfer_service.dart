...
class RelayMediaTransferService {
  ...
  Future<RelayMediaRestoreResult> restoreIncomingMedia({
    ...
    if (blob.isNotFound) {
      final elapsedMs = DateTime.now().difference(startedAt).inMilliseconds;
      _log(
        'restore not-found peer=$peerId messageId=$messageId '
        'blobId=$blobId elapsedMs=$elapsedMs',
      );
  ...
  Future<_RelayBlobDownloadRetryResult> _downloadBlobWithRetry({
    ...
    for (var attempt = 0; attempt < attempts; attempt++) {
      ...
      if (download != null) {
        _log(
          'download attempt complete peer=$peerId messageId=$messageId '
          'blobId=$blobId attempt=${attempt + 1}/$attempts '
          'bytes=${download.payload.length} notFound=${download.isNotFound}',
        );
...
class RelayMediaRetryCoordinator {
  ...
  bool isDue(String key) {
    ...
    final lastErrorKind = state?['lastErrorKind'];
    if (attemptsForKey(key) >= maxAttempts && lastErrorKind != 'unavailable') {
      return false;
    }
  ...
  Future<bool> recordFailure({
    ...
    states[key] = <String, dynamic>{
      ...
      'nextRetryAtMs': canRetry ? nowMs + backoffMs : 0,
  ...
  void scheduleRetry({
    required String peerId,
    required String messageId,
    required Future<RelayRetryMessage?> Function(
      String peerId,
      String messageId,
    )
    findMessage,
    ...
    if (attemptsForKey(key) >= maxAttempts &&
        state?['lastErrorKind'] != 'unavailable') {
      return;
    }
    final nextRetryAtMs = _nextRetryAtMsForKey(key);
    final nowMs = DateTime.now().millisecondsSinceEpoch;
    ...
    _timers[key] = Timer(Duration(milliseconds: delayMs), () async {
      ...
      try {
        ...
        if (transferId.startsWith('dirblob:')) {
          restoreInBackground(peerId, messageId, isGroup: false);
        } else if (transferId.startsWith('grpblob:')) {
          restoreInBackground(peerId, messageId, isGroup: true);
        }
      } catch (error) {
        AppFileLogger.log(
