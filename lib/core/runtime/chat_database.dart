...
class ChatSummaries extends Table {
  ...
  BoolColumn get hasMoreMessages =>
...
class ChatDatabase extends _$ChatDatabase {
  ChatDatabase({required String directory})
    : super(_openChatDatabase(directory));
      ...
      if (from < 2) {
        ...
        await customStatement('''
          INSERT OR REPLACE INTO chat_messages_v2
            (id, message_id, peer_id, timestamp_ms, payload_json)
          SELECT id, message_id, peer_id, timestamp_ms, payload_json
          FROM chat_messages
          ORDER BY id
        ''');
  ...
  Future<List<Map<String, dynamic>>> getAllChatSummariesAsJson() async {
    final rows =
        await (select(chatSummaries)..orderBy([
              (t) => OrderingTerm(
  ...
  Future<void> deleteMessagesByIds(
    ...
    await (delete(
          chatMessages,
        )..where((t) => t.peerId.equals(peerId) & t.messageId.isIn(messageIds)))
        .go();
  }
