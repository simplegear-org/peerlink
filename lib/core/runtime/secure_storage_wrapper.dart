...
class SecureStorageWrapper {
  ...
  static Future<void> initialize({
    ...
    try {
      ...
      debugPrint('[SecureStorageWrapper] File storage directory ready: ${_fileStorageDirectory!.path}');
      ...
      _initialized = true;
      debugPrint('[SecureStorageWrapper] Initialized (useFileStorage: $_useFileStorage)');
    } catch (e, stack) {
      debugPrint(
        '[SecureStorageWrapper] Failed to initialize: $e\n$stack',
      );
      _useFileStorage = true;
      _initialized = true;
    }
  ...
  static Future<void> _tryEnableSecureStorage() async {
    try {
      debugPrint('[SecureStorageWrapper] Attempting to initialize secure storage...');
  ...
  static Future<String?> read(String key) async {
    if (!_initialized) {
      throw StateError('SecureStorageWrapper.initialize() must be called first');
    }
    ...
    if (_useFileStorage || _secureStorage == null) {
      ...
      debugPrint('[SecureStorageWrapper] Read from file: ${result != null ? "${result.length} bytes" : "null"}');
      return result;
    }
    ...
    try {
      ...
      debugPrint('[SecureStorageWrapper] Read from secure storage: ${result != null ? "${result.length} bytes" : "null"}');
      if (result != null) {
        return result;
      }
  ...
  static Future<void> write(String key, String value) async {
    ...
    debugPrint('[SecureStorageWrapper] Writing key: $key (${value.length} bytes), useFileStorage: $_useFileStorage');
  ...
  static Future<void> clear() async {
    if (_secureStorage != null && !_useFileStorage) {
      debugPrint('[SecureStorageWrapper] Clear not supported for secure storage');
    }
