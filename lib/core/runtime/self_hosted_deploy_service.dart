...
class SelfHostedDeployService {
  ...
  Future<SelfHostedDeployResult> deploy({
    ...
    if (normalizedHost.isEmpty) {
      throw const FormatException(
        'Укажите только домен или IP сервера без пути, query и символов вроде #',
      );
    }
    ...
    _emitStage(onProgress, 1, 'Connect to server');
  ...
  String _buildDeployCommand({
    ...
    final escapedPublicHost = _shellEscape(publicHost);
  ...
  Future<void> _waitForServiceReadiness({
    ...
    while (true) {
      ...
      try {
        await action();
        if (attempt > 1) {
          _log(onProgress, '$label ready after retry #$attempt');
        }
        return;
        ...
        _log(onProgress, '$label not ready yet, retry #$attempt: $error');
  ...
  bool _isSafeSelfHostedHost(String host) {
