...
class PushServerEntry {
  ...
  const PushServerEntry({required this.endpoint, this.paused = false});

  PushServerEntry copyWith({String? endpoint, bool? paused}) {
  ...
  Map<String, dynamic> toJson() {
    return <String, dynamic>{'endpoint': endpoint, 'paused': paused};
  }

  static PushServerEntry? fromStorage(Object? raw) {
    if (raw is String) {
      final normalized = PushServersService.normalizeIncomingEndpoint(raw);
    ...
    if (raw is Map) {
      ...
      final normalized = PushServersService.normalizeIncomingEndpoint(
...
class PushServersService implements ServerAvailabilityProvider {
  ...
  List<String> get endpoints =>
      List<String>.unmodifiable(entries.map((item) => item.endpoint));
  ...
  static String normalizeIncomingEndpoint(String value) {
  ...
  Future<ServerAvailability> _probeEndpoint(String endpoint) async {
    ...
    try {
      final healthStatusCode =
          await ServerRuntimeUtils.requestStatusWithTimeout(
            client: client,
