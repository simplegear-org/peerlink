...
class AudioCallPeer {
  ...
  set _lastInboundVideoBytes(int value) =>
      _videoState.lastInboundVideoBytes = value;
  int get _lastInboundVideoFramesDecoded =>
      _videoState.lastInboundVideoFramesDecoded;
  ...
  Future<void> _preparePeerConnection() async {
    ...
    final config = await _negotiationController.buildRtcConfig(
    ...
    final localTracks = List<MediaStreamTrack>.from(_localStream!.getTracks());
  ...
  void _armIceFailureTimer(Timer? currentTimer, Duration grace, String error) {
    if (!_connected) {
      _onError(error);
      return;
    }
  ...
  Future<void> _syncLocalMediaTracks() async {
    ...
    if (_mediaType == CallMediaType.video && videoTracks.isEmpty) {
      ...
      for (final track in sourceTracks) {
        final alreadyAttached = stream.getVideoTracks().any(
          (existing) => existing.id == track.id,
        );
        if (!alreadyAttached) {
          stream.addTrack(track);
    ...
    if (_mediaType == CallMediaType.audio && videoTracks.isNotEmpty) {
      ...
      await _disposeVideoSourceStream();
      _log(
        'video:sender detached localTracks=${stream.getVideoTracks().length}',
      );
  ...
  void _log(String message) {
    AppFileLogger.log(
      '[call_peer][${_peerId ?? 'unknown'}][${_logSeq++}] $message',
    );
  }
}
