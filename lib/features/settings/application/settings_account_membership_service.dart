...
import 'package:peerlink/core/runtime/account_device_event.dart';
...
class SettingsAccountMembershipService {
  ...
  const SettingsAccountMembershipService({
    ...
    required this.sendAccountMembershipUpdatePushEvent,
  ...
  Future<void> revokeAccountDevices(Iterable<String> targetDeviceIds) async {
    ...
    for (final peerId in recipientPeerIds) {
      ...
      try {
        await sendAccountMembershipUpdatePushEvent(
          directPeerId: peerId,
          update: payload,
        );
      } catch (error, stackTrace) {
