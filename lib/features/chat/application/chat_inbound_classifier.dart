...
class ChatInboundClassifier {
  ...
  final DecodePayload decodeGroupKeyRequestPayload;
  final DecodePayload decodeDirectBlobRefPayload;
  final DecodePayload decodeGroupBlobRefPayload;
  final DecodePayload decodeMessageReceiptPayload;
  ...
  const ChatInboundClassifier({
    ...
    required this.decodeGroupKeyRequestPayload,
    required this.decodeDirectBlobRefPayload,
    required this.decodeGroupBlobRefPayload,
    required this.decodeMessageReceiptPayload,
  ...
  IncomingChatDispatch classifyIncomingMessage(ChatMessage msg) {
    ...
    final groupKeyRequest = normalizeGroupKeyRequestPayload(msg.text);
    if (groupKeyRequest != null) {
      return IncomingGroupKeyRequestDispatch(groupKeyRequest);
    }
    ...
    final receipt = normalizeMessageReceiptPayload(msg.text);
    if (receipt != null) {
      return IncomingMessageReceiptDispatch(receipt);
    }
  ...
  IncomingMessageReceiptPayload? normalizeMessageReceiptPayload(String text) {
    final raw = decodeMessageReceiptPayload(text);
    if (raw == null) {
      return null;
    }
    final chatId = (raw['chatId'] as String? ?? '').trim();
    final status = (raw['status'] as String? ?? '').trim();
    final senderPeerId = (raw['senderPeerId'] as String? ?? '').trim();
    final atMsRaw = raw['atMs'];
    final atMs = atMsRaw is int ? atMsRaw : int.tryParse('${atMsRaw ?? ''}');
    final messageIds = _readStringList(raw['messageIds']);
    if (chatId.isEmpty ||
    ...
    return IncomingMessageReceiptPayload(
      chatId: chatId,
      isGroup: raw['chatKind'] == 'group',
  ...
  IncomingGroupChatDeletePayload? normalizeGroupChatDeletePayload(String text) {
  ...
  IncomingGroupKeyRequestPayload? normalizeGroupKeyRequestPayload(String text) {
    final raw = decodeGroupKeyRequestPayload(text);
    if (raw == null) {
      return null;
    }
    final groupId = (raw['groupId'] as String? ?? '').trim();
    final ownerPeerId = (raw['ownerPeerId'] as String? ?? '').trim();
    final requesterPeerId = (raw['requesterPeerId'] as String? ?? '').trim();
    final failedMessageId = (raw['failedMessageId'] as String? ?? '').trim();
    if (groupId.isEmpty || ownerPeerId.isEmpty || requesterPeerId.isEmpty) {
      return null;
    }
    return IncomingGroupKeyRequestPayload(
      groupId: groupId,
