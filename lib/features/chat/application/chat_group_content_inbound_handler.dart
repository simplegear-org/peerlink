...
import 'package:peerlink/features/chat/application/chat_group_control_inbound_handler.dart';
...
class ChatGroupContentInboundHandler {
  ...
  Future<void> handleIncomingGroupSecureMessage(
    ...
    if (clearText == null || clearText.isEmpty) {
      ...
      throw GroupInboundDeferredException(
        'group secure decrypt failed group=$groupId messageId=${msg.id}',
      );
    }
    logGroupFlow(
  ...
  Future<void> handleIncomingGroupBlobRef(
    ...
    if (blobRef.contentKind == 'text') {
      ...
      if (text == null || text.isEmpty) {
        ...
        throw GroupInboundDeferredException(
          'group blob text restore failed group=$groupId messageId=${blobRef.messageId} blob=${blobRef.blobId}',
        );
      }
      final incoming = Message(
...
class GroupInboundDeferredException implements Exception {
  final String reason;

  const GroupInboundDeferredException(this.reason);

  @override
  String toString() => 'GroupInboundDeferredException($reason)';
}
