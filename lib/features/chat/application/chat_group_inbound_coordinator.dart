...
class ChatGroupInboundCoordinator {
  ...
  Future<void> handleSecureDecryptFailed({
    ...
    final ownerPeerId = knownGroupOwnerPeerId(groupId)?.trim();
    if (ownerPeerId != _facade.peerId) {
      if (ownerPeerId == null || ownerPeerId.isEmpty) {
        AppFileLogger.log(
          '[chat_group] group key request skip reason=unknown-owner '
          'group=$groupId source=$sourcePeerId messageId=$messageId',
          name: 'chat',
        );
        return;
      }
      final request = _outboundCodec.encodeGroupKeyRequestPayload(
        groupId: groupId,
        ownerPeerId: ownerPeerId,
        failedMessageId: messageId,
      );
      await _facade.sendControlMessage(
        ownerPeerId,
        kind: 'groupKeyRequest',
        text: request,
      );
      AppFileLogger.log(
        '[chat_group] group key request sent group=$groupId '
        'owner=$ownerPeerId source=$sourcePeerId messageId=$messageId',
        name: 'chat',
      );
      return;
    }
  ...
  Future<void> handleKeyRequest(
    ...
    final resolvedPayload =
        payload ?? _inboundClassifier.normalizeGroupKeyRequestPayload(msg.text);
    if (resolvedPayload == null) {
      return;
    }
    final groupId = resolvedPayload.groupId;
    final requesterPeerId = resolvedPayload.requesterPeerId.trim();
    if (resolvedPayload.ownerPeerId != _facade.peerId ||
        requesterPeerId.isEmpty) {
      AppFileLogger.log(
        '[chat_group] group key request drop group=$groupId '
        'owner=${resolvedPayload.ownerPeerId} requester=$requesterPeerId',
        name: 'chat',
      );
      return;
    }
    ...
    if (groupChat == null || !groupChat.isGroup) {
      AppFileLogger.log(
        '[chat_group] group key request drop reason=unknown-group '
        'group=$groupId requester=$requesterPeerId',
        name: 'chat',
      );
      return;
    }
    final members = groupChat.memberPeerIds
        .map((item) => item.trim())
        .where((item) => item.isNotEmpty)
        .toSet();
    if (!members.contains(requesterPeerId)) {
      AppFileLogger.log(
        '[chat_group] group key request drop reason=requester-not-member '
        'group=$groupId requester=$requesterPeerId',
        name: 'chat',
      );
      return;
    }
    await _groupFlowService.sendGroupKeyToRecipients(groupChat, <String>[
      requesterPeerId,
    ]);
    AppFileLogger.log(
      '[chat_group] group key request answered group=$groupId '
      'requester=$requesterPeerId failedMessageId=${resolvedPayload.failedMessageId}',
      name: 'chat',
    );
  }
