...
import 'package:peerlink/features/contacts/infrastructure/contact_name_resolver.dart';
...
class IosCallkitFacadeAdapter {
  ...
  factory IosCallkitFacadeAdapter.fromNodeFacade(NodeFacade facade) {
    return IosCallkitFacadeAdapter(
      ...
      registerPushDeviceToken: (token) => facade.registerPushDeviceToken(token),
    );
  }
}
...
class IosCallkitService {
  IosCallkitService._({
    ...
    bool Function()? isIosOverride,
           ...
           methodChannel ?? const MethodChannel('peerlink/callkit/methods'),
       _eventChannel =
  ...
  factory IosCallkitService.test({
    ...
    final service = IosCallkitService._(
      methodChannel: methodChannel,
      eventChannel: eventChannel,
  ...
  Future<void> initializeWithAdapter(
    ...
    _callStateSubscription ??= facade.getCallStateStream().listen(
      _syncCallState,
    );
  ...
  Future<void> _registerVoipTokenIfNeeded(
    ...
    try {
      ...
      if (storage == null || pushTokens == null) {
        return;
      }
      await storage.init();
      await pushTokens.saveVoipToken(normalized);
    } catch (_) {}
    try {
  ...
  Future<String?> _resolveLocalContactName(String peerId) async {
    ...
    try {
      ...
      await storage.init();
      final contacts = storage.getContacts();
