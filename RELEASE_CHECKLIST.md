...
- [ ] For iOS, after changing `pubspec.yaml`, run `flutter build ios --config-only --no-codesign` or a full iOS build so `ios/Flutter/Generated.xcconfig` gets the current `FLUTTER_BUILD_NAME` / `FLUTTER_BUILD_NUMBER`
...
- [ ] For local VSCode launches, verify the device is selected in the device picker and the launch profile does not require a manual `deviceId` in `.vscode/launch.json`
...
- [ ] Check the final Android manifest/APK does not include unused sensitive permissions (`USE_EXACT_ALARM`, `MANAGE_EXTERNAL_STORAGE`, storage/media, contacts/call-log), unless they are core release functionality
